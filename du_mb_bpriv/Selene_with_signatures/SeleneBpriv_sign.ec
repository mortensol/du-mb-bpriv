...
module type VFR_adv(H:HOracle.POracle, G:GOracle.POracle) = {
  proc main (pk: (epkey * PKEtr.pkey * aux)) 
...
module type VotingRelationS (Ev:PKEvo.Scheme, H:HOracle.POracle) = {
  proc main(s:((epkey * PKEtr.pkey * aux) * 
            ...
            w:(((ident,opening) fmap * eskey * skey) * 
               ((ident * upkey * supkey * commitment) * cipher * signature) list)
            ) : bool
}. 
...
module VFRS (Ev:PKEvo.Scheme, A:VFR_adv, R:VotingRelationS, CP:CommitmentProtocol,
  ...
  proc main() : bool = {
    var r', r, t, i, j, rel, b, id, et1, et2, et3, ev, s, v, tr, c;
    var bb, rL;
   ...
   while (i < card BP.setidents) {
    ...
    rel <@ R(Ev,H).main(((vpk,tpk,(BP.setidents, tpTr, pTr, pPk, pCo,pi1,pi2)), sPublish bb, r), 
                       ((pOp,vsk,tsk), bb));
...
module BCCA(V:VotingSystem, SS: SignatureScheme, CP:CommitmentProtocol, 
            A:DU_MB_BPRIV_adv, S:Simulator, IO:Ind1CCA_Oracles) = {

    var bb : (ident * (ident * upkey * supkey * commitment) * cipher * signature) list
    ...
    module O = {
      ...
      proc vote(id:ident, v0 v1 : vote) = {
        ...
        if (id \in BP.setH) {
          c <@ IO.enc(id, v0, v1);
          s <@ SS.sign( (oget BP.secmap.[id]).`2, c);
          BP.vmap.[id] <- (c, (c,s), v0, v0) :: (odflt [] BP.vmap.[id]);
          bb <- (id, oget BP.pubmap.[id], c, s) :: bb;
        }
      }
  ...
  proc main(pk:epkey) : bool = {
   ...
   while (i < card BP.setidents) {
    ...
    (* register *)
      ...
      cL <- map (fun x: ((ident * upkey * supkey * commitment) * cipher*signature) 
                     => (x.`2, x.`1.`1)) BP.bb;
      ...
      while (j < size BP.bb) {
        ...
        if ( (ev, id) \in BS.encL) {
          ...
          k  <- find (fun (x : cipher * (cipher*signature) * vote * vote) 
                           => x.`1 = ev) sv; (* first index where b appears *)
...
(** Signature Scheme **)
declare axiom SS_gen_ll    : islossless SS.gen. 
declare axiom SS_sign_ll   : islossless SS.sign. 
declare axiom SS_verify_ll : islossless SS.verify. 
...
declare axiom SS_s_eq (ge: (glob SS)):
   phoare[SS.sign : (glob SS) = ge ==> (glob SS) = ge] = 1%r. 
...
lemma in_rem_ids (id:ident) 
                 (pc:ident*upkey*supkey*commitment) (c:cipher) (s: signature) 
                 (bb:(ident*(ident*upkey*supkey*commitment)* cipher * signature) list) 
       : (id, pc, c, s) \in bb => (pc, c, s) \in rem_ids bb.  
proof. 
move => in_bb. 
rewrite /rem_ids.
...
(* Operator removing the last element of a four tuple *)
op drop_last_of4 (x : ('a * 'b * 'c * 'd)) = (x.`1, x.`2, x.`3). 
...
module (BZK(Ev:PKEvo.Scheme, P:Prover, C:ValidIndS, Ve:Verifier, 
           A:DU_MB_BPRIV_adv, CP:CommitmentProtocol, SS: SignatureScheme): VotingAdvZK)
           (H:HOracle.Oracle, G:GOracle.POracle) = {

    module O   = DU_MB_BPRIV_oracles(Selene(Ev,P,Ve,C,CP,SS),H,G,Left)
    module Smv = Selene(Ev,P,Ve,C,CP,SS, H,G)

    proc a1() : ((epkey * pkey * aux) * 
                 ((ident * upkey * supkey * commitment) * cipher*signature) list * 
                 (vote list * tracker list)) * 
                (((ident,opening) fmap * eskey * skey) *
                 ((ident * upkey * supkey * commitment) * cipher*signature) list) = {

      var i, id, ct, d, upk, vt;
      var rL, j, b, ev, v, tr', supk, susk, s;
...
auto;progress => /#.   
wp. 
call(A_a1_ll (<: BZK(Ev,P,C,Ve,A,CP,SS,H,G).O)).
proc;inline*.
if => //;wp;call SS_sign_ll; call(Ev_enc_ll H Ho_ll);
         wp;call SS_sign_ll; call (Ev_enc_ll H Ho_ll);wp;auto. 
proc;inline*;auto. 
while (0 <= i <= card BP.setidents) (card BP.setidents - i);
  progress;auto. call{1} SS_gen_ll;auto. smt(). 
...
seq 3 : true => //. 
inline*.
wp;call(PS_v_ll G Go_ll);wp;call(A_a2_ll (<: BZK(Ev,P,C,Ve,A,CP,SS,H,G).O)). 
proc;inline*;auto.  
auto;progress. 
call(A_a4_ll (<: BZK(Ev,P,C,Ve,A,CP,SS,H,G).O));auto. 
if => //; first rnd;skip;progress. rewrite dboolE => />.  
seq 1 : valid' => //. 
call(A_a3_ll (<: BZK(Ev,P,C,Ve,A,CP,SS,H,G).O)). 
proc;inline*;auto. 
if =>//.
...
module BVFR(V:VotingSystem, A:DU_MB_BPRIV_adv, 
            CP:CommitmentProtocol, SS: SignatureScheme, 
...
local module G0L' (Ev:PKEvo.Scheme, P:Prover, Ve:Verifier, C:ValidIndS, CP:CommitmentProtocol,
                   SS: SignatureScheme, A:DU_MB_BPRIV_adv, H:HOracle.Oracle, G:GOracle.Oracle) = {
    ...
    proc main() : bool = {
      var i;
      var id, upk, ct, d, da, supk, susk;
      var valid, valid';
      var rL, j, b, ev, v, tr', vt, s;
...
seq 34 34 : (={glob CP, glob A, glob C, glob P, glob Ev, glob G, glob HRO.ERO, glob Ve, glob SS,
...
call(_: ={HRO.ERO.m, glob G, glob SS, BP.secmap, BP.pubmap, 
         BP.setH, BP.pk, glob Ev, BP.bb0, BP.bb1, BP.vmap}); [1..4: by sim].  

while ( ={i, glob SS, BP.setidents, BP.pk, BP.sk, glob CP, BP.secmap, BP.pubmap, BP.setD, BP.secmapD});
  first sim. 
  wp; call(: true).
  by auto. 
...
wp.  call(_: ={glob G});first by sim. 
wp;rnd. 
while (={rL, glob Ev, glob HRO.ERO} /\ i2{1} = j{2} 
        /\ BP.bb{2} = bb0{1} /\ sd0{1} = BP.sk{2} /\ pd1{1} = BP.pk{2}); first by sim. 
auto;progress. 
qed. 
...
module G0L(Ev:PKEvo.Scheme, P:Prover, Ve:Verifier, C:ValidIndS, CP:CommitmentProtocol,
  ...
  proc main() : bool = {
      var i;
      var id, upk, ct, d, da, vt, supk, susk, s;
      var valid, valid';
      var rL, j, b, ev, v, tr';
...
seq 14 14 : (={glob CP, glob A, glob C, glob P, glob Ve, glob Ev, glob G, glob SS,
               BP.setHcheck, BP.secmapD, BP.setD, BP.setidents, BP.setH, BP.pk, BP.sk, 
               BP.vmap, BP.pubmap, BP.bb,
...
local module G1L(Ev:PKEvo.Scheme, Ve:Verifier, C:ValidIndS, CP:CommitmentProtocol,
                 SS: SignatureScheme, A:DU_MB_BPRIV_adv, H:HOracle.Oracle, S:Simulator) = {

  module O = DU_MB_BPRIV_oracles(Selene(Ev,P,Ve,C,CP,SS),H,S,Left)
  module A = A(O)
  module Smv = Selene(Ev,P,Ve,C,CP,SS,H,S)
...
local module ZKFL(Ev:PKEvo.Scheme, R:VotingRelationS, P:Prover, 
                   A:VotingAdvZK, H:HOracle.Oracle, G:GOracle.Oracle) = {
    proc main() : bool = {
      var p;
...
local module ZKFR(Ev:PKEvo.Scheme, R:VotingRelationS, A:VotingAdvZK, 
...
local lemma ZKFL_ZKL &m : 
     BP.setidents{m} = BP.setH{m} `|` BP.setD{m} =>
    `|Pr[ZKFL(Ev,R,P,BZK(Ev,P,C,Ve,A,CP,SS),HRO.ERO,G).main() @ &m : res] -
      Pr[ZK_L(R(Ev,HRO.ERO),P,BZK(Ev,P,C,Ve,A,CP,SS,HRO.ERO), G).main() @ &m : res]| <=
      Pr[ZK_L(R(Ev,HRO.ERO),P,BZK(Ev,P,C,Ve,A,CP,SS,HRO.ERO), G).main() @ &m : !BPS.rel] .  
proof. 
move => ?. 
byequiv (_: ={glob A, glob C, glob CP, glob R, glob P, glob Ve, 
              glob Ev, glob G, glob SS, BP.setHcheck, BP.secmapD, BP.pubmap, BP.secmap,
              BP.setD, BP.setidents, BP.d, BP.setH, BP.bb0, BP.bb1, BP.r, BP.sethappy, 
              glob HRO.ERO, BP.setchecked, BP.sk,
              BP.vmap, BP.pk, BP.bb} ==> 
...
proc. 
seq 3 3 : (
    ={glob P, glob SS, glob R, glob HRO.ERO, glob G, glob A, glob Ev, 
      glob C, glob CP, glob Ve, BPS.s, BPS.w, BPS.rel,
      BP.secmapD, BP.setD, BP.setidents, BP.setH, BP.bb0, BP.bb1, BP.setHcheck, 
      BP.r, BP.d, BP.pubmap, BP.secmap, BP.sk,
      BP.sethappy, BP.setchecked, BP.vmap,BP.pk, BP.bb}
).
call(_: ={glob HRO.ERO, glob Ev}); first by sim.
call(_: ={glob HRO.ERO, glob G, glob SS, glob A, glob Ev, glob CP, 
          BP.secmapD, BP.setD, BP.setidents, BP.pk, BP.bb, BP.pubmap, BP.setHcheck,
          BP.secmap, BP.setH, BP.bb0, BP.bb1, BP.r, BP.sethappy, BP.setchecked, BP.vmap,
          BP.sk, BP.d}). sim.  
...
local lemma ZKFR_ZKR &m : 
     BP.setidents{m} = BP.setH{m} `|` BP.setD{m} =>
    `|Pr[ZKFR(Ev,R,BZK(Ev,P,C,Ve,A,CP,SS),HRO.ERO,S).main() @ &m : res] -
      Pr[ZK_R(R(Ev,HRO.ERO),S,BZK(Ev,P,C,Ve,A,CP,SS,HRO.ERO)).main() @ &m : res]| <=
      Pr[ZK_R(R(Ev,HRO.ERO),S,BZK(Ev,P,C,Ve,A,CP,SS,HRO.ERO)).main() @ &m : !BPS.rel] .  
proof. 
move => ?. 
byequiv (_: ={glob A, glob C, glob CP, glob SS, glob R, glob S, glob Ve,
              glob Ev, BP.setHcheck, BP.secmapD, BP.pubmap, BP.secmap,
              BP.setD, BP.setidents, BP.d, BP.setH, BP.bb0, BP.bb1, BP.r, 
              BP.sethappy, glob HRO.ERO, BP.setchecked, BP.sk,
              BP.vmap, BP.pk, BP.bb} ==> 
              ={BPS.rel} /\ (BPS.rel{2} => ={res})) : (!BPS.rel) => //;last first. 
...
proc. 
seq 3 3 : (
    ={glob S, glob R, glob HRO.ERO, glob SS, glob A, glob Ev, glob C, 
      glob CP, glob Ve, BPS.s, BPS.w, BPS.rel, BP.sk,
      BP.secmapD, BP.setD, BP.setidents, BP.setH, BP.bb0, BP.bb1, 
      BP.setHcheck, BP.r, BP.d, BP.pubmap, BP.secmap,
      BP.sethappy, BP.setchecked, BP.vmap,BP.pk, BP.bb}
).
call(_: ={glob HRO.ERO, glob Ev}); first by sim. 
call(_: ={glob HRO.ERO, glob S, glob A, glob SS, glob Ev, glob CP, 
          BP.secmapD, BP.setD, BP.setidents, BP.pk, BP.bb, BP.pubmap, BP.d,
          BP.secmap, BP.setH, BP.bb0, BP.bb1, BP.r, BP.sethappy, BP.setchecked, 
          BP.vmap, BP.sk, BP.setHcheck}). sim. 
call(_:true). skip;progress. 
if{2} => //; first by sim. 
...
local lemma ZKL_rel &m (H <: HOracle.Oracle {-A, -BPS, -BP, -BS, -Ve, -Ev, -C, -R, -CP, -SS})
  ...
  islossless H.o => islossless G.o => islossless P(G).prove =>
    ...
    <= Pr[VFRS(Ev, BVFR(Selene(Ev,P,Ve,C,CP,SS), A, CP,SS), R, CP,SS, H, G).main() @ &m : res]. 
proof. 
move => ? Ho_ll Go_ll Pp_ll.
byequiv (_: ={glob A, glob Ev, glob P, glob CP, glob Ve, glob C, 
              glob H, glob G, glob R, glob SS, BP.setHcheck,
...
call(_: ={BP.pk, BP.secmapD, BP.pubmap, BP.setH, BP.secmap, BP.bb0, BP.bb1,
          glob Ev, glob P, glob Ve, glob C, glob CP, glob H, glob G, glob SS}); [1..4: by sim]. 
while (i{1} = i0{2} /\ ={BP.setidents, glob CP, glob SS, BP.secmap, 
...
have H1 : Pr[ZK_L(R(Ev,HRO.ERO),P,BZK(Ev,P,C,Ve,A,CP,SS,HRO.ERO), G).main() @ &m : !BPS.rel] <= 
          Pr[VFRS(Ev,BVFR(Selene(Ev,P,Ve,C,CP,SS),A,CP,SS),R,CP,SS,HRO.ERO,G).main() @ &m : res]. 
...
local lemma ZKR_rel &m (H <: HOracle.Oracle {-A, -BPS, -BP, -BS, -Ve, -Ev, -C, -CP, -SS, -R})
     ...
     Pr[VFRS(Ev, BVFR(Selene(Ev,P,Ve,C,CP,SS),A,CP,SS), R, CP, SS, H, S).main() @ &m : res]. 
proof. 
move => ? Ho_ll So_ll Sp_ll.
byequiv (_: ={glob A, glob Ev, glob S, glob CP, glob Ve, glob C, glob H, glob R, glob SS, BP.setHcheck,
              BP.setidents, BP.setD, BP.secmapD, BP.setH, BP.bb0, BP.bb1} ==> _ ) => //.
proc. 
...
auto. call(_: ={glob H}); first by sim. auto;progress. 
 wp.  
call(_: ={BP.pk, BP.secmapD, BP.pubmap, BP.setH, BP.secmap, BP.bb0, BP.bb1,
          glob Ev, glob Ve, glob C, glob CP, glob H, glob S, glob SS}); [1..4: by sim]. 
while (i{1} = i0{2} /\ ={BP.setidents, glob CP, glob SS, BP.secmap, 
                        BP.pubmap, BP.setD, BP.secmapD, BP.sk, BP.pk}); first by sim. 
wp.  
while (={BP.setidents, tpk, pTr, pPk, pOp, pPk, tpTr, pCo, tsk, pi2} 
...
have H1 : Pr[ZK_R(R(Ev,HRO.ERO),S,BZK(Ev,P,C,Ve,A,CP,SS,HRO.ERO)  ).main() @ &m : !BPS.rel] <= 
          Pr[VFRS(Ev,BVFR(Selene(Ev,P,Ve,C,CP,SS),A,CP,SS),R,CP,SS,HRO.ERO,S).main() @ &m : res]. 
...
seq 34 34 : (
  ={glob HRO.ERO, glob A, glob C, glob CP, glob P, glob Ve, glob Ev, glob G, glob SS,
...
seq 13 14 : (
  ={BP.setHcheck, BP.vmap, valid, BP.setchecked, BP.sethappy, BP.bb0, BP.bb1, 
    ...
    glob HRO.ERO, glob A, glob G, glob Ev, glob P, glob Ve, glob C, glob CP, glob SS, valid} 
...
(* Everything up to creating BP.bb *)
seq 34 34 : (
  ={glob HRO.ERO, glob A, glob C, glob CP, glob Ve, glob Ev, glob S, glob SS,
    BP.setHcheck, BP.secmapD, BP.setD, BP.setidents, BP.setH, BP.bb0, BP.bb1, BP.bb,
    BP.sethappy, BP.vmap, BP.pk, BP.sk, BP.setchecked, BP.secmap, BP.pubmap}
); first by sim. 
swap{2} [11..18] -10. 
...
seq 13 14 : (
  ={BP.setHcheck, BP.vmap, valid, BP.setchecked, BP.sethappy, BP.bb0, BP.bb1, 
    BP.r, BP.bb, BP.secmap, BP.pubmap, BP.pk, BP.sk, BP.setidents, vt,
    glob HRO.ERO, glob A, glob S, glob Ev, glob Ve, glob C, glob CP, glob SS, valid} 
    /\ (BPS.s{2} = (BP.pk{2}, sPublish BP.bb{2}, BP.r{2}))
    /\ (BPS.w{2} = (BP.sk{2}, BP.bb{2}))
). 
wp.
...
have H1 := H0 Pr[ZKFL(Ev, R, P, BZK(Ev, P, C, Ve, A, CP,SS), HRO.ERO, G).main() @ &m : res]
              Pr[ZKFR(Ev, R, BZK(Ev, P, C, Ve, A, CP,SS), HRO.ERO, S).main() @ &m : res]
              Pr[ZK_L(R(Ev, HRO.ERO), P, BZK(Ev, P, C, Ve, A, CP, SS,HRO.ERO), G).main() @ &m : res]
              Pr[ZK_R(R(Ev, HRO.ERO), S, BZK(Ev, P, C, Ve, A, CP, SS,HRO.ERO)).main() @ &m : res]. 
...
local module G2L (Ev:PKEvo.Scheme, Ve:Verifier, C:ValidIndS, CP:CommitmentProtocol, 
  ...
  module O = {
    ...
    var p, b, spr, spo,a;  
      ...
      (p, b, a, spr, spo) <@ Smv.vote(BP.pk, id, oget BP.pubmap.[id], sl, v);
    
      return (p, b, a, spr, spo);
    ...
    proc vote(id:ident, v0 v1 : vote) = {
      ...
      if (id \in BP.setH) {
        (p, b, a, spr, spo) <@ vote_core(id, v0, oget BP.secmap.[id]);

        BP.vmap.[id] <- (b, spr, spo, v0) :: (odflt [] BP.vmap.[id]);

        bb <- (id, oget BP.pubmap.[id], b, a) :: bb; 
      }
    }
      ...
      valid <@ Smv.validboard(BP.bb, BP.pk);
       ...
       (* We now tally *)
          ...
          while (j < size BP.bb) {
            ...
            (ev,s) <- (b.`2,b.`3);
            id     <- b.`1.`1;
            if ( (id, oget BP.pubmap.[id], ev) \in  (map drop_last_of4 bb)) {
...
proof. 
move => ?. 
byequiv (_: ={glob A, glob C, glob CP, glob Ve, glob S, glob Ev, glob SS, 
              BP.setHcheck, BP.secmapD, BP.setD,
              BP.setidents, BP.setH} ==> _) => //.
proc. 

(******** Everything up until the adversary creates BP.bb is more or less equivalent **********)
seq 14 13 : (
  ={glob A, glob C, glob CP, glob Ve, glob S, glob Ev, glob SS, glob HRO.ERO, 
    BP.setHcheck, BP.secmapD, BP.setD, BP.setidents, BP.setH, BP.vmap, BP.pk, 
    BP.sk, BP.bb, BP.sethappy, BP.setchecked, BP.pubmap, BP.secmap}
    /\  ( BP.bb0{1} = G2L.bb{2})

    /\ (forall (id:ident) pc c s, (id, pc, c, s) \in G2L.bb{2}  =>  
            Some (oget (onth (odflt [] BP.vmap{2}.[id]) 
                                          (find (fun (x: cipher*(cipher*signature)*vote*vote) => x.`1 = c) 
                                               (odflt [] BP.vmap{2}.[id])))).`4
             = dec_cipher_vo BP.sk{2}.`2 id c HRO.ERO.m{2})

    /\ (forall (id:ident) pc c s, (id, pc, c, s) \in G2L.bb{2} => 
                                          (pc, c, s) \in rem_ids G2L.bb{2})

    /\ (forall (id:ident) c s pc, 
                 (id, pc, c, s) \in ( BP.bb0{1}) => 
                    is_some BP.vmap{1}.[id])

    /\ (forall id pc c s, (id, pc, c, s) \in G2L.bb{2} => pc = oget BP.pubmap{2}.[id])
).

call(_: ={glob HRO.ERO, glob S, glob C, glob CP, glob SS, glob Ev, BP.pk, BP.secmapD, BP.pubmap, BP.sk,
          BP.secmap, BP.setH, BP.setidents, BP.vmap} /\ (BP.bb0{1} = G2L.bb{2})
         /\ (BP.pk{1}.`1 = get_epk BP.sk{1}.`2)

         /\ (forall (id:ident) c s pc, (id, pc ,c, s) \in G2L.bb{2} => 
            Some (oget (onth (odflt [] BP.vmap{2}.[id]) 
           (find (fun (x : (cipher*(cipher*signature)*vote*vote)) => x.`1 = c) 
                (odflt [] BP.vmap{2}.[id])))).`4
             = dec_cipher_vo BP.sk{2}.`2 id c HRO.ERO.m{2})
...
+ proc. 
  inline*. 
  if => //. 
  seq 14 10 : (={id, v0, v1, id1, ev,s, id0, HRO.ERO.m, 
     glob S, glob C, glob CP, glob SS, glob Ev, BP.pk, BP.secmapD, BP.pubmap, BP.sk, 
    ...
    /\ (forall (id3 : ident) c s pc,
      (id3, pc, c, s) \in G2L.bb{2} =>
      Some
        (oget
  ...
  + sp. 
    ...
    elim em_kp.  
    + progress. 
      call(: true); call (Eenc_dec_vo sk1.`2 pk1.`1 id1 v1); first by auto. 
      by auto;progress.  
    move => h. 
    conseq (_: _ ==> !(pk1.`1 = get_epk sk1.`2));progress. 
    call{1} SS_sign_ll; call{2} SS_sign_ll.
    call{1} (Ev_enc_ll HRO.ERO HRO.ERO_o_ll); call{2} (Ev_enc_ll HRO.ERO HRO.ERO_o_ll). 
    by auto;progress. 
  wp.  
  exists* (glob Ev){1}, (glob SS){1}; elim* => gev gss. 
  call{1} (SS_s_eq gss); call{1} (Ev_e_eq gev); wp.
  ...
  + simplify. 
    ...
    have H10 : !(find (fun (x : cipher * (cipher*signature) * vote * vote) 
                        => x.`1 = c) (odflt [] BP.vmap{2}.[id3]) + 1 = 0). 
    + have ? : 0 <= (find (fun (x:cipher*(cipher*signature)*vote*vote) 
                        => x.`1 = c) (odflt [] BP.vmap{2}.[id3])) by rewrite find_ge0. 
      rewrite addz1_neq0 => //.   
    rewrite -ideq H10. 
    simplify. 
    have H11 : (id3, pc1, c, s1) \in (G2L.bb{2}) by smt().    
    apply H1 in H11; first smt(). 

  + rewrite get_set_neqE.  rewrite ideq => />.  
    have H10 : (id3, pc1, c, s1) \in (G2L.bb{2}) by smt(). 
    apply H1 in H10.  rewrite H10 => />.

  + case(id3 = id{2}) => ideq. 
    rewrite get_set_eqE //=.
    rewrite get_set_neqE //=.  rewrite (H2 id3 c s1 pc1). smt(). 
...
while (={i, BP.setidents, glob CP, glob SS, BP.secmap, 
         BP.pubmap, BP.setD, BP.secmapD, BP.pk, BP.sk, BP.setH} 
). sim.
auto. wp. 
inline*.
...
trivial. rewrite (in_rem_ids id1 pc c3 s0) => />. 
  
(**** The ifs are equivalent ***)
seq 6 6 : (
  ={glob HRO.ERO, glob Ev, glob S, glob Ve, glob A, glob CP, 
    BP.setHcheck, BP.vmap, BP.pk, BP.sk, BP.bb, BP.setchecked, BP.setidents, vt,
...
seq 4 4 : (
    (={glob HRO.ERO, glob Ev, glob S, glob Ve, glob A, glob CP, 
       BP.setHcheck, BP.bbstar, BP.r, BP.pubmap, BP.secmap, BP.vmap, BP.setidents, 
...
wp. call(_: ={glob HRO.ERO, glob S} 
              /\ BP.bb0{1} = G2L.bb{2}); [proc;inline*;auto | sim | sim | auto;progress]. 
call(_: ={glob HRO.ERO, glob S}); [1..2:by sim].  
call(_:true). 
auto;progress. 
if => //; first rnd;progress. 
seq 1 1 : (((={glob HRO.ERO, glob Ev, glob S, glob Ve, glob A, glob CP, 
        BP.setHcheck, BP.pubmap, BP.secmap, BP.vmap, BP.setidents,
...
call(_: ={BP.bb, BP.bbstar, BP.pubmap, BP.secmap, BP.vmap, glob CP, 
          glob HRO.ERO, glob S, BP.setchecked, BP.sethappy, BP.setidents}).
sim. sim. sim. auto;progress. 
...
while (={j, BP.bb, glob HRO.ERO, glob Ev, BP.vmap, rL, BP.sk, BP.pk, BP.pubmap}
  /\ (forall (id0 : ident) c s,
    (id0, oget BP.pubmap{2}.[id0], c, s) \in G2L.bb{2} =>
    Some
      (oget
  ...
  /\  (forall (id0 : ident) c s pc,
    (id0, pc, c, s) \in G2L.bb{2} =>
    (pc, c, s) \in rem_ids G2L.bb{2})
...
+ call(_:true);first auto. 
  ...
  call(Edec_Odec_eq_vo sk.`2 id ev). 
  auto;progress =>/#. 
...
move: H5; rewrite mapP /drop_last_of4 //= ; elim => x [Hxm [Hx1 [Hx2 Hx3]]].
rewrite Hx1 Hx3.
have Ho:= (H x.`1 x.`3 x.`4 _). 
+ by rewrite -Hx1 Hx2; smt(). 
smt().

inline*. wp. 
while (={i0, bb, e1, e2, e3, glob HRO.ERO, glob C, pd, BP.pubmap}). sim;progress. 
auto;progress. 
+ rewrite (H id00 (oget BP.pubmap{2}.[id00]) c s1) => />. 
- by rewrite (in_rem_ids id00 pc c s1).   
qed. 
...
local module G0R' (Ev:PKEvo.Scheme, P:Prover, Ve:Verifier, C:ValidIndS, CP:CommitmentProtocol,
                   SS: SignatureScheme, A:DU_MB_BPRIV_adv, H:HOracle.Oracle, 
                   G:GOracle.Oracle, S:Simulator) = {

    module O   = DU_MB_BPRIV_oracles(Selene(Ev,P,Ve,C,CP,SS), H, S, Right)
    module A   = A(O)
    module Smv = Selene(Ev,P,Ve,C,CP,SS,H,S)
...
local lemma DU_MB_BPRIV_R_G0R'_equiv &m : 
  BP.setidents{m} = BP.setH{m} `|` BP.setD{m} =>
    ...
    = Pr[G0R'(Ev,P,Ve,C,CP,SS,A,HRO.ERO,G,S).main() @ &m : res]. 
proof.  
move => ?. 
byequiv => //. 
proc;inline*.  
...
seq 35 35 : (
  ={glob HRO.ERO, glob S, glob C, glob CP, glob SS, glob P, glob A, 
    glob Ev, glob G, glob Ve, BP.bb, BP.bb0, BP.bb1, BP.pk, BP.sk, BP.setidents, 
...
seq 8 8 : (
    ={glob HRO.ERO, glob S, glob C, glob CP, glob SS, glob P, glob A, 
      glob Ev, glob Ve, glob G, BP.setidents, 
...
seq 31 26 : (
  (={glob HRO.ERO, glob S, glob C, glob CP, glob SS, glob P, glob A, 
     glob Ev, glob Ve, glob G, valid', BP.r, BP.pi, BP.pi', BP.bbstar, BP.setidents,
...
seq 1 1 : (
  ((={glob HRO.ERO, glob S, glob C, glob CP, glob SS,  glob P, glob A, BP.d, BP.bbstar,
...
call(_: ={glob HRO.ERO, BP.bbstar, BP.bb, BP.vmap, BP.secmap, BP.pubmap, BP.setidents, 
          BP.sethappy, BP.setchecked, glob CP, glob SS, glob S}); [1..3: by sim]; first auto.  
if => //;sp. if => //;first rnd;progress. 
if => //; first rnd;progress.  
...
call(_: ={glob HRO.ERO, glob S, BP.secmap, BP.pubmap, BP.setH, BP.pk, 
         glob SS, glob Ev, BP.vmap, BP.bb0, BP.bb1});first sim; progress. 
sim;progress.  sim. sim. 
while (={i, BP.setidents, BP.pk, BP.sk, glob CP, glob SS, 
         BP.secmap, BP.pubmap, BP.setD, BP.secmapD}). 
 + sim; wp; call(: true); auto;progress.
...
local lemma G0R'_G0R_equiv &m : 
    BP.setidents{m} = BP.setH{m} `|` BP.setD{m} =>
    Pr[G0R'(Ev,P,Ve,C,CP,SS,A,HRO.ERO,G,S).main() @ &m : res] = 
    Pr[G0R(Ev,Ve,C,CP,SS,A,HRO.ERO,S).main() @ &m : res]. 
proof. 
move => ?. 
byequiv => //. 
proc.   
...
seq 15 14 : (
  ={glob HRO.ERO, glob CP, glob A, glob C, glob Ve, glob S, glob Ev, glob SS,
...
call(_:
  ={glob HRO.ERO, glob S, glob Ev, glob SS, BP.pk, BP.sk, BP.pubmap, BP.secmap, BP.setH, BP.vmap,
    BP.bb0, BP.bb1}
); [1..4: by sim].  
while(={i, BP.setidents, glob CP, glob SS, BP.pubmap, BP.setD, 
        BP.secmapD, BP.sk, BP.secmap, BP.pk}); first sim. progress.   
wp. 
inline*.   
wp;while(={i0, BP.setidents, tpk, tsk, pPk, pTr, pCo, pOp, trL, pi2}); first sim.  
wp;while(={i0, BP.setidents, trL, pTr, glob CP, pPk, tpk}); first sim. 
...
auto;progress; [rewrite size_ge0 | smt() | rewrite size_ge0 | smt() | 
                rewrite size_ge0 | smt() | rewrite duni_ap_ll].
...
(* The valid checks are equivalent *)
seq 1 1 : (={glob CP, glob A, glob C, glob Ve, glob Ev, glob S, glob SS,
      BP.setHcheck, BP.secmapD, BP.setD, BP.setidents, BP.setH, BP.pk, BP.sk,
      BP.vmap, BP.pubmap, BP.secmap, BP.bb0, BP.bb1, BP.setchecked, BP.bb,
...
wp;while{2} (0 <= i0{2} <= size bb{2}) (size bb{2} - i0{2});progress;auto;progress; 
    [smt() | smt() | smt() | rewrite size_ge0 | smt() | 
      rewrite  size_ge0 | smt() | rewrite duni_ap_ll].
...
(* by sim. *)
wp; rnd.
while (={j, BP.bb', glob Ev, HRO.ERO.m, BP.sk, BP.pk, rL}).
 + by sim; auto.  
wp; while (={i0, bb, bb1, bb', bb0}).
+ sim.
by auto.
qed. 
...
local module G1R(Ev:PKEvo.Scheme, Ve:Verifier, C:ValidIndS, CP:CommitmentProtocol, 
    ...
    module O = {
      ...
      proc vote(id:ident, v0 v1 : vote) = {
      var p0, b0, a0, s0pr, s0po;
      var p1, b1, a1, s1pr, s1po;
  
      if (id \in BP.setH) {
        (p0, b0, a0, s0pr, s0po) <@ vote_core(id, v0, oget BP.secmap.[id]);
        (p1, b1, a1, s1pr, s1po) <@ vote_core(id, v1, oget BP.secmap.[id]);

        BP.vmap.[id] <- (b0, s1pr, s0po, v0) :: (odflt [] BP.vmap.[id]);

        BP.bb0 <- (id, oget BP.pubmap.[id], b0, a0) :: BP.bb0;
        BP.bb1 <- (id, oget BP.pubmap.[id], b1, a1) :: BP.bb1;
        }
      }
    ...
    proc main() : bool = {
      ...
      var id, upk, ct, d, da, vt;
      var valid, valid';
      var rL, j, b, ev, v, tr', sv, k, supk, susk, s;
      ...
      while (j < size BP.bb') {
        ...
        (ev,s)   <- (b.`2,b.`3);
        id   <- b.`1.`1;
        if ( (id, oget BP.pubmap.[id], ev) \in (map drop_last_of4 BP.bb0)) {
          (* Get the vote from BP.vmap that mathes the ballot *)
          sv <- odflt [] BP.vmap.[id]; (* get list for voter id *)
          k  <- find (fun (x :cipher * (cipher*signature) * vote * vote) 
                      => x.`1 = ev) sv; (* first index where b appears *)
...
local lemma G0R_G1R_equiv &m : 
    BP.setidents{m} = BP.setH{m} `|` BP.setD{m} =>
    Pr[G0R(Ev,Ve,C,CP,SS,A,HRO.ERO,S).main() @ &m : res] = 
    Pr[G1R(Ev,Ve,C,CP,SS,A,HRO.ERO,S).main() @ &m : res]. 
proof. 
move => ?. 
byequiv => //. 
proc. 
...
seq 14 14 : ( 
  ={glob CP, glob A, glob C, glob Ve, glob S, glob SS, glob Ev, glob HRO.ERO, 
    BP.sethappy, BP.setchecked, BP.setidents, 
    BP.setHcheck, BP.secmapD, BP.setD, BP.setidents, BP.setH, BP.pubmap, BP.secmap, 
    BP.bb0, BP.bb1, BP.bb, BP.pk, BP.sk}
  /\ (BP.pk{1}.`1 = get_epk BP.sk{1}.`2) 
  /\ (fdom BP.vmap{1} = fdom BP.vmap{2})
  /\ (forall id i, rem_fst4 (oget (onth (odflt [] BP.vmap{1}.[id]) i)) = 
                   rem_fst4 (oget (onth (odflt [] BP.vmap{2}.[id]) i)))
  /\ (forall (id : ident) c s pc,  (id, pc, c, s) \in BP.bb0{2}  =>  
       Some (oget (onth (odflt [] BP.vmap{2}.[id]) 
               (find (fun (x : cipher * (cipher*signature) * vote * vote) => x.`1 = c) 
               (odflt [] BP.vmap{2}.[id])))).`4
        = dec_cipher_vo BP.sk{2}.`2 id c HRO.ERO.m{2})
  /\ (forall (id:ident) c s pc, (id, pc, c,s) \in BP.bb0{1} => is_some BP.vmap{1}.[id])
  /\ (forall id pc c s, (id, pc, c, s) \in BP.bb0{1} => pc = oget BP.pubmap{2}.[id])
).
...
call(_: ={glob HRO.ERO, glob S, glob C, glob CP, glob SS, glob Ev, BP.pk, BP.secmapD, BP.pubmap, BP.sk,
    ...
    /\ (forall id i, rem_fst4 (oget (onth (odflt [] BP.vmap{1}.[id]) i)) = 
                     rem_fst4 (oget (onth (odflt [] BP.vmap{2}.[id]) i)))
    /\ (forall (id : ident) c s pc,  
          (id, pc, c,s) \in BP.bb0{2}  =>  
               Some (oget (onth (odflt [] BP.vmap{2}.[id]) 
                              (find (fun (x : cipher * (cipher*signature) * vote * vote) 
                                     => x.`1 = c) (odflt [] BP.vmap{2}.[id])))).`4
        = dec_cipher_vo BP.sk{2}.`2 id c HRO.ERO.m{2})
    /\ (forall (id:ident) c s pc, (id, pc, c,s) \in BP.bb0{1} => is_some BP.vmap{1}.[id])
    /\ (forall id pc c s, (id, pc, c, s) \in BP.bb0{1} => pc = oget BP.pubmap{2}.[id])
).
+ proc. 
  ...
  wp.
  call(: true).
  call(: ={glob HRO.ERO}); first by sim.
  wp; call(: true).
  sp; exists* (glob Ev){1}, BP.sk{1}, pd{1}, id1{1}, v{1}.
    elim* => ev sk pd id v.
  ...
  + move => h. 
      ...
      + rewrite id_eq. 
        ...
        - case (is_some BP.vmap{1}.[id4]);move => vmap1_is_some.
          + have vmap2_is_some : is_some BP.vmap{2}.[id4]; first by smt(@SmtMap). 
            have is_some_odflt : 
              forall (a : ((cipher * (cipher*signature) * vote * vote) list) option), 
    ...
    + (** Proof that vote stored in vmap is decryption of stored ciphertext **)
      ...
      + rewrite id_eq. 
        ...
        case (result_R = c); move => ev2_c_eq. 
        + rewrite ev2_c_eq //=. smt(@SmtMap). 
        rewrite ev2_c_eq //=.
        have->: !(find (fun (x : cipher * (cipher * signature) * vote *vote) => x.`1 = c) 
                       (odflt [] BP.vmap{2}.[id4])
                  + 1 = 0).
        + smt(find_ge0).
        rewrite //=.
        have in_bb0 : (id4, pc1, c,s1) \in BP.bb0{2} by smt(). 
        by rewrite (H2 id4 c s1 pc1 in_bb0).
      - rewrite get_set_neqE; first by smt().
        rewrite (H2 id4 c s1 pc1); first by smt().
        done. 

    + (* is_some BP.vmap{1} *)
      case(id4 = id) => ideq.  
      + by rewrite get_set_eqE 1: ideq.  
      - rewrite get_set_neqE 1: ideq //=.  
        by rewrite (H3 id4 c s1 pc1); first by smt(). 
...
while ( ={i, BP.setidents, glob CP, glob SS, BP.secmap, BP.pubmap, BP.setD, 
             BP.secmapD, BP.sk, BP.pk} ); first sim. 
wp;inline*.   
 wp;while(={i0, BP.setidents, tpk, tsk, pPk, pTr, pCo, pOp, trL, pi2}); first sim.  
wp;while(={i0, BP.setidents, trL, pTr, glob CP, pPk, tpk}); first sim. 
wp;rnd;while(={i0, BP.setidents, tpk, tpTr, trL}); first sim. 
...
seq 7 7 : (
  ={glob HRO.ERO, glob S, glob Ve, glob CP, glob SS, BP.setHcheck, valid, 
    glob A, BP.r,  BP.bb0, BP.bb1, BP.pk, BP.bb, BP.bb', BP.setidents,
...
seq 13 13 : (
  ((={HRO.ERO.m, glob S, glob Ve, glob CP, BP.setHcheck, valid, glob A, 
     BP.r, BP.pi', BP.bb0, BP.bb1, BP.pk, BP.bb, valid', BP.bbstar, BP.setidents,
...
seq 1 1 : (
   (((={HRO.ERO.m, glob S, glob Ve, glob CP, BP.setHcheck, valid, 
        glob A, BP.r, BP.pi', BP.d, BP.bbstar, da, BP.setidents, 
         BP.bb0, BP.bb1, BP.pk, BP.bb, valid', BP.pubmap, BP.secmap, BP.sethappy, BP.setchecked} /\
     fdom BP.vmap{1} = fdom BP.vmap{2}) 
       /\ (forall id i, rem_fst4 (oget (onth (odflt [] BP.vmap{1}.[id]) i)) = 
           rem_fst4 (oget (onth (odflt [] BP.vmap{2}.[id]) i)))
   /\ (BP.setHcheck{1} \subset fdom BP.vmap{1})) /\
...
while (
  ...
  (forall (id0 : ident) c s,
     (id0, oget BP.pubmap{2}.[id0], c, s) \in BP.bb0{2} =>
     Some
       (oget
          (onth (odflt [] BP.vmap{2}.[id0])
             (find (fun (x : cipher* (cipher*signature) * vote * vote) => x.`1 = c)
...
auto;progress. 
- rewrite (H2 id00 c s1 (oget BP.pubmap{2}.[id00])). exact/H9.  
  done.  
qed. 
...
local module G2R(Ev:PKEvo.Scheme, Ve:Verifier, C:ValidIndS, CP:CommitmentProtocol, 
    ...
    module O = {
      ...
      proc verify = G1R(Ev,Ve,C,CP,SS,A,H,S).O.verify
    ...
    proc main() : bool = {
      ...
      while (j < size BP.bb) {
        ...
        (ev,s)   <- (b.`2, b.`3);
        id   <- b.`1.`1;
        if ( (id, oget BP.pubmap.[id], ev) \in (map drop_last_of4 BP.bb1)) {
          (* Get the vote from BP.vmap that mathes the ballot *)
          sv <- odflt [] BP.vmap.[id]; (* get list for voter id *)
          k  <- find (fun (x : cipher * (cipher*signature) * vote * vote) 
                => x.`1 = ev) sv; (* first index where b appears *)
...
local lemma G1R_G2R_equiv &m : 
    BP.setidents{m} = BP.setH{m} `|` BP.setD{m} =>
    Pr[G1R(Ev,Ve,C,CP,SS,A,HRO.ERO,S).main() @ &m : res] = 
    Pr[G2R(Ev,Ve,C,CP,SS,A,HRO.ERO,S).main() @ &m : res]. 
proof. 
move => id_union. 
byequiv => //;proc. 
...
seq 13 13 : (
  ={glob A, glob C, glob CP, glob Ve, glob S, glob SS, glob Ev, HRO.ERO.m, 
    BP.setHcheck, BP.secmapD, BP.pk, BP.sk, BP.setidents, 
    BP.setD, BP.setidents, BP.setH, BP.vmap, BP.pubmap, BP.secmap, BP.bb0, 
    BP.bb1, BP.setchecked, BP.sethappy}
  ...
  /\ (forall id pc c s, (id, pc, c, s) \in BP.bb1{1} => pc = (oget BP.pubmap{1}.[id]))
). 
 
while(={i, BP.setidents, BP.pubmap, BP.secmap, BP.secmapD, BP.setD, BP.sk, glob CP, glob SS, BP.pk}

  /\ (forall j id, j <= i{1} => id \in (take j (elems BP.setidents{1})) => is_some BP.pubmap{1}.[id])
...
seq 7 7 : (
  ...
  + wp; call(: true).
    auto;progress.   
    + rewrite get_setE. 
      case( id0 = nth witness (elems BP.setidents{2}) i{2}); first by smt(). 
      move => id0_neq_nth. 
...
seq 1 1 : (
  ...
  ={glob A, glob S, glob C, glob CP, glob Ev, glob Ve, glob HRO.ERO, 
    ...
    BP.bb0, BP.bb1, BP.bb, BP.sethappy, BP.setchecked, BP.setH, BP.secmap, BP.secmapD, BP.pubmap}

  /\ (* 1 *) (BP.pk{1}.`1 = get_epk BP.sk{1}.`2)

  /\ (* 2 *) (forall id pc c s, (id, pc, c, s) \in BP.bb0{1} => pc = oget BP.pubmap{1}.[id])
  /\         (forall id pc c s, (id, pc, c, s) \in BP.bb1{1} => pc = oget BP.pubmap{1}.[id])
  ...
  /\ (* 5 *) (forall (id:ident) c pc, (id, pc, c) \in (map drop_last_of4 BP.bb0{1}) => 
  ...
  /\ (* 9 *) (forall (id:ident) c pc, (id, pc, c) \in (map drop_last_of4 BP.bb1{2}) =>
             Some (oget (onth (odflt [] BP.vmap{1}.[id]) 
                    (find (fun (x:cipher*(cipher*signature)*vote*vote) => 
                             x.`1 = (nth witness BP.bb0{2} (index (id, pc, c) 
                                    (map drop_last_of4 BP.bb1{2}))).`3)
                 (odflt [] BP.vmap{1}.[id])))).`4
             = Some (oget (onth (odflt [] BP.vmap{2}.[id]) 
                    (find (fun (x:cipher*(cipher*signature)*vote*vote) => x.`1 = c)
                 (odflt [] BP.vmap{2}.[id])))).`4)   

  /\ (* 10 *) (forall b, b \in rem_ids BP.bb0{1} <=> (b.`1.`1, b.`1, b.`2, b.`3) \in BP.bb0{1} ) 
  /\          (forall b, b \in rem_ids BP.bb1{1} <=> (b.`1.`1, b.`1, b.`2, b.`3) \in BP.bb1{1} ) 

  /\ (* 11 *) (forall b, b \in BP.bb1{2} => b.`2.`1 = b.`1)

  /\ (* 12 *) (forall b, b \in (map drop_last_of4 BP.bb1{2}) => 
                index b (map drop_last_of4 BP.bb1{2}) 
                 = index (b.`2, b.`3) (map drop_last_of3 (rem_ids BP.bb1{2})))
...
(* A.a1 call *)
call (_: 
  ={glob S, glob C, glob CP, glob Ev, glob SS, glob Ve, glob HRO.ERO, 
    BP.pk, BP.sk, BP.setHcheck, BP.setidents, 
    BP.bb0, BP.bb1, BP.sethappy, BP.setchecked, BP.setH, BP.secmap, BP.secmapD, BP.pubmap}

  /\ (BP.pk{1}.`1 = get_epk BP.sk{1}.`2)

  /\ (forall id pc c s, (id, pc, c, s) \in BP.bb0{1} => pc = oget BP.pubmap{1}.[id])
  /\ (forall id pc c s, (id, pc, c, s) \in BP.bb1{1} => pc = oget BP.pubmap{1}.[id])
  ...
  /\ (forall (id:ident) c pc, (id, pc, c) \in (map drop_last_of4 BP.bb0{1}) => 
          Some (oget (onth (odflt [] BP.vmap{1}.[id]) 
          (find (fun (x : cipher* (cipher*signature) * vote * vote) => x.`1 = c) 
          (odflt [] BP.vmap{1}.[id])))).`4
           = dec_cipher_vo BP.sk{2}.`2 id c HRO.ERO.m{2})
  ...
  /\ (forall (id:ident) c pc, (id, pc, c) \in (map drop_last_of4 BP.bb1{2}) =>
           Some (oget (onth (odflt [] BP.vmap{1}.[id]) 
                 (find (fun (x:cipher*(cipher*signature)*vote*vote) => 
                 x.`1 = (nth witness BP.bb0{2} (index (id, pc, c) (map drop_last_of4 BP.bb1{2}))).`3)
                 (odflt [] BP.vmap{1}.[id])))).`4
           = Some (oget (onth (odflt [] BP.vmap{2}.[id]) 
                  (find (fun (x:cipher*(cipher*signature)*vote*vote) => x.`1 = c)
                  (odflt [] BP.vmap{2}.[id])))).`4) 

  /\ (forall (id:ident) c s pc, (id, pc, c, s) \in BP.bb0{1} \/ (id, pc, c, s) \in BP.bb1{1}
  ...
  /\ (forall b, b \in rem_ids BP.bb0{1} <=> (b.`1.`1, b.`1, b.`2, b.`3) \in BP.bb0{1} )
  /\ (forall b, b \in rem_ids BP.bb1{1} <=> (b.`1.`1, b.`1, b.`2, b.`3) \in BP.bb1{1} ) 

  /\ (forall b, b \in BP.bb1{2} => b.`2.`1 = b.`1)

  /\ (forall b, b \in (map drop_last_of4 BP.bb1{2}) => 
                index b (map drop_last_of4 BP.bb1{2}) 
                = index (b.`2, b.`3) (map drop_last_of3 (rem_ids BP.bb1{2})))
...
swap{1} [14..21] -5. swap{2} [14..21] -5. 

seq 23 23 : (
  ={id, v0, s0, v1, v3, id2, pd, ev, v4, pd0, id3, b0, ev0, s0po, a0, s0} 
    /\ v3{1} = v0{1} /\ v4{1} = v1{1} /\ b0{1} = ev{1} /\

  ={glob S, glob C, glob CP, glob Ev, glob Ve, glob SS, HRO.ERO.m, BP.pk, BP.pubmap,
   ...
   (forall (id4 : ident) c  pc, (id4, pc, c) \in (map drop_last_of4 BP.bb0{1}) =>
      Some (oget (onth (odflt [] BP.vmap{1}.[id4]) 
           (find (fun (x:cipher*(cipher*signature) * vote * vote) => x.`1 = c)
           (odflt [] BP.vmap{1}.[id4])))).`4 =  dec_cipher_vo BP.sk{2}.`2 id4 c HRO.ERO.m{2}) /\
   ...
   (forall (id4 : ident) c pc, (id4, pc, c) \in (map drop_last_of4 BP.bb1{2}) =>
     Some (oget (onth (odflt [] BP.vmap{1}.[id4]) 
          (find (fun (x : cipher*(cipher*signature) * vote * vote) =>
        x.`1 = (nth witness BP.bb0{2} (index (id4, pc, c) (map drop_last_of4 BP.bb1{2}))).`3)
...
wp; call(: true); call(_: ={glob HRO.ERO}). sim. auto; progress. 
...
have in_bb0 : (id4, pc1, c) \in (map drop_last_of4 BP.bb0{2}) by smt(). 
...
have H36 : !(find (fun (x:cipher*(cipher*signature)*vote*vote) => x.`1 = c) 
                  (odflt [] BP.vmap{1}.[id4]) + 1 = 0).  
have H22 : 0 <= find (fun (x:cipher*(cipher*signature)*vote*vote) => x.`1 = c) 
...
have in_bb0 : (id4, pc1, c) \in (map drop_last_of4 BP.bb0{2}) by smt(). 
rewrite get_set_neqE; first by smt(). 
rewrite (H4 id4 c pc1); first rewrite in_bb0.  
done.
...
(* property 7 has 2 invariants, this is second *)
have -> : (rem_ids ((id{2}, oget BP.pubmap{2}.[id{2}], ev{2}, a0{2}) :: BP.bb0{2}))
          = (oget BP.pubmap{2}.[id{2}], ev{2}, a0{2}) :: (rem_ids BP.bb0{2}) by smt(in_rem_ids).  
have -> : (rem_ids ((id{2}, oget BP.pubmap{2}.[id{2}], ev0{2}, s0{2}) :: BP.bb1{2})) 
          = (oget BP.pubmap{2}.[id{2}], ev0{2},s0{2}) :: (rem_ids BP.bb1{2}) by smt(in_rem_ids).   
rewrite -cat1s nth_cat //=. 
case (i=0); first by done. 
rewrite -(H7 (i-1)). 
smt(@List). 

(* 1 + size BP.bb0{2} = 1 + size BP.bb1{2} *)
by rewrite H8. 

(* Property 9: recovery and vmap on left side is equvalent to vmap on the right *)
elim : H21 =>//= [[Hid [Hpc Hc]] | Hr].
+ move: Hid Hpc Hc ; rewrite //=; move => Hid Hpc Hc.
  rewrite Hid.
  rewrite !get_set_sameE.
  rewrite -Hc -Hpc. 
  by rewrite index_head //=.

(* (id4, pc1, c) \in (map drop_last_of BP.bb1{2}) reduce to H9 *)

case (id4 <> id{2}) => Hid.
+ rewrite !get_set_neqE 1: Hid //=. 
  rewrite -cat1s index_cat //=.
  have->: drop_last_of4 (id{2}, oget BP.pubmap{2}.[id{2}], ev0{2}, s0{2})
          = (id{2}, oget BP.pubmap{2}.[id{2}], ev0{2}) by rewrite /drop_last_of4.
  rewrite //= Hid //=.
  have ->: !((1 + index (id4, pc1, c) (map drop_last_of4 BP.bb1{2})) = 0)
    by smt(add1z_neq0 index_ge0). 
  rewrite //=.
  by rewrite (H9 id4 c pc1 Hr).

move: Hid; rewrite //=; move => Hid.
rewrite -Hid !get_set_sameE.

have Hpc: pc1 = oget BP.pubmap{2}.[id4].
+ move: Hr; rewrite (mapP drop_last_of4 BP.bb1{2} (id4, pc1, c)). 
  rewrite /drop_last_of4 //=; elim => x [Hxmem Hxeq]. 
  smt().

rewrite -Hpc.
case (ev0{2} = c) => Hc.
+ rewrite -Hc.
  have->: index (id4, pc1, c)
                 (drop_last_of4 (id4, pc1, c, s0{2}) :: 
                 map drop_last_of4 BP.bb1{2})
          = 0
    by rewrite -/drop_last_of4 //= index_head.
  by done.

have ->: index (id4, pc1, c)
               (drop_last_of4 (id4, pc1, ev0{2}, s0{2})
                :: map drop_last_of4 BP.bb1{2})
         = 1 + index (id4, pc1, c)  (map drop_last_of4 BP.bb1{2}).
+ rewrite -cat1s index_cat /drop_last_of4 //=; smt(). 
have ->: ! (1 + index (id4, pc1, c) (map drop_last_of4 BP.bb1{2}) = 0)
  by smt(index_ge0).
rewrite //=.
...
pose i:= (index (id4, pc1, c) (map drop_last_of4 BP.bb1{2})).
have Hisize: i < size BP.bb1{2}.
+ smt (@List).

case (ev{2} = (nth witness BP.bb0{2} i).`3) => Hev //=.
+ rewrite -(H9 _ _ _ Hr).
  have Hdec := H4 id4 ev{2} pc1  _.
  + have Hid_bb0: id4 = (nth witness BP.bb0{2} i).`1.
    + rewrite H5.  
      have := (nth_map witness witness drop_last_of4 i BP.bb1{2} _).
      + by rewrite Hisize /i index_ge0.      
      rewrite {2}/drop_last_of4 //=.
      smt(@List). 
    have Hpc_bb0: pc1 = (nth witness BP.bb0{2} i).`2.
    + rewrite H6.  
      have := (nth_map witness witness drop_last_of4 i BP.bb1{2} _).
      + by rewrite Hisize /i index_ge0.      
      rewrite {2}/drop_last_of4 //=.
...
move: H21; rewrite /rem_ids -cat1s map_cat mem_cat //=.
rewrite -/rem_ids.
elim => [Hl | Hr].
+ left.
  have Hsome: is_some BP.pubmap{2}.[id{2}].
  + rewrite (H14 id{2} _). 
    + by rewrite in_fsetU H10. 
  have:= H15 id{2} Hsome.
  smt().
right.
exact H16. 
...
move: H21; elim => [Hl|Hr].
+ rewrite /rem_ids /rem_id -cat1s map_cat mem_cat //=.
  smt().
smt().
...
elim: H21 => [Hl|Hr].
+ have : id{2} = (oget BP.pubmap{2}.[id{2}]).`1.
  + have Hsome: is_some BP.pubmap{2}.[id{2}].
    + rewrite (H14 id{2} _). 
      + by rewrite in_fsetU H10. 
    have:= H15 id{2} Hsome.
...
have->: (rem_ids ((id{2}, oget BP.pubmap{2}.[id{2}], ev0{2}, s0{2}) :: BP.bb1{2}))
        =  ( oget BP.pubmap{2}.[id{2}], ev0{2}, s0{2}) :: (rem_ids BP.bb1{2}).
+ by rewrite /rem_ids /rem_id /map_cons.
have ->: (drop_last_of4 (id{2}, oget BP.pubmap{2}.[id{2}], ev0{2}, s0{2}))
         = (id{2}, oget BP.pubmap{2}.[id{2}], ev0{2}).
+ by rewrite /drop_last_of4. 
rewrite map_cons.
rewrite ?index_cons.
elim: H21 => [Hl|Hr].
+ smt().
rewrite -(H19 b5 Hr). 
have Hid : id{2} = (oget BP.pubmap{2}.[id{2}]).`1.
+ have Hsome: is_some BP.pubmap{2}.[id{2}].
  + rewrite (H14 id{2} _). 
    + by rewrite in_fsetU H10. 
  have:= H15 id{2} Hsome.
  smt().
rewrite /drop_last_of3 //=. 
case (b5.`2 = oget BP.pubmap{2}.[id{2}] /\ b5.`3 = ev0{2}) => Hb5.
+ have ->: b5 = (id{2}, oget BP.pubmap{2}.[id{2}], ev0{2}).
  + move: Hr. rewrite mapP /drop_last_of4 //=. elim => x [Hxmem Hxb5].
    have := H18 x Hxmem. 
    smt(). 
  smt().
smt().
...
have ->: (rem_ids ((id{2}, oget BP.pubmap{2}.[id{2}], ev{2}, a0{2}) :: BP.bb0{2}))
         = (oget BP.pubmap{2}.[id{2}], ev{2}, a0{2}) :: (rem_ids BP.bb0{2}).
  by rewrite /rem_ids map_cons /rem_id.
rewrite -cat1s (nth_cat witness) //=.
smt().
...
seq 1 1 : (
 ={glob A, glob S, glob C, glob CP, glob Ev, glob Ev, glob Ve, glob HRO.ERO,
  ...
  (forall (id0 : ident) (c : cipher), 
          (id0, oget BP.pubmap{2}.[id0], c) \in (map drop_last_of4 BP.bb0{1}) =>
          Some (oget (onth (odflt [] BP.vmap{1}.[id0]) 
               (find (fun (x : cipher * (cipher*signature) * vote * vote) => x.`1 = c)
               (odflt [] BP.vmap{1}.[id0])))).`4 =
               dec_cipher_vo BP.sk{2}.`2 id0 c HRO.ERO.m{2}) /\

  (forall (i0 : int), (nth witness BP.bb0{2} i0).`1 = (nth witness BP.bb1{2} i0).`1) /\

  (forall (i0 : int), (nth witness BP.bb0{2} i0).`2 = (nth witness BP.bb1{2} i0).`2) /\
  (forall (i0 : int), (nth witness (rem_ids BP.bb0{1}) i0).`1 = 
                      (nth witness (rem_ids BP.bb1{1}) i0).`1) /\

  size BP.bb0{2} = size BP.bb1{2} /\
  
  (forall id0 c, (id0, oget BP.pubmap{2}.[id0], c) \in (map drop_last_of4 BP.bb1{2})
       => Some (oget (onth (odflt [] BP.vmap{1}.[id0])
                           (find (fun (x : cipher * (cipher*signature) * vote * vote)
                                 => x.`1 
                                    = (nth witness BP.bb0{2}
                                           (index (id0, oget BP.pubmap{2}.[id0], c)
                                                  (map drop_last_of4  BP.bb1{2}))).`3)
                                 (odflt [] BP.vmap{1}.[id0])))).`4 
  ...
  /\ (valid{2} => forall pc c s, (pc, c, s) \in BP.bb{2} => (oget BP.pubmap{2}.[pc.`1]) = pc)
).

+ inline*.  
  wp;
  while(={i0, bb, e1, e2, e3, glob C, glob HRO.ERO, pd, BP.pubmap}
        /\ (e3{2} =>  
              forall j pc c s, j <= i0{2} => 
              (pc, c, s) \in (take j bb{2}) 
            => oget BP.pubmap{2}.[pc.`1] = pc)).  

    seq 9 9 : (={i0, bb, e1, e2, e3, glob C, glob HRO.ERO, pd, BP.pubmap} 
           /\ (e3{2} => 
                  forall (j0 : int) (pc : ident * upkey * supkey * commitment) (c:cipher) (s:signature),
                     j0 <= i0{2} =>
                     (pc, c, s) \in take j0 bb{2} 
                     => oget BP.pubmap{2}.[pc.`1] = pc)
           /\ ((id0{2}, upk0{2}, supk0{2}, ct0{2}), ev0{2}, s0{2}) = nth witness bb{2} i0{2}
           /\  i0{1} < size bb{1} 
           /\  i0{2} < size bb{2} 
           /\ (e3{2} = (oget BP.pubmap{2}.[id0{2}] = (id0{2}, upk0{2}, supk0{2}, ct0{2}))) ). 
    + wp. 
      call(_: ={glob HRO.ERO}); first by sim. 
      wp. 
      skip. progress. 
...
rewrite (H17 H20 i0_R pc c s1).  trivial. smt(@List). trivial. 
...
seq 3 2 : (
  ={glob A, glob S, glob C, glob CP, glob Ev, glob Ve, glob HRO.ERO, 
    BP.pubmap, BP.secmap, BP.setidents, BP.sk, BP.setHcheck, rL, valid, 
    BP.bb0, BP.bb1, BP.bb, BP.sethappy, BP.setchecked, j, BP.pk}
  /\ (j{1} = 0)
  /\ (rL{1} = [])
  ...
  /\ (forall (id:ident) (c:cipher), 
             (id, oget BP.pubmap{1}.[id], c) \in (map drop_last_of4 BP.bb0{1}) =>
          Some (oget (onth (odflt [] BP.vmap{1}.[id]) 
               (find (fun (x : cipher*(cipher*signature)*vote*vote) => x.`1 = c)
               (odflt [] BP.vmap{1}.[id])))).`4
    ...
    (forall i, !(nth witness (map drop_last_of3 BP.bb{2}) i \in (map drop_last_of3 (rem_ids BP.bb1{1}))) 
  ...
  /\ (forall (id:ident) (c:cipher), 
        (id, oget BP.pubmap{2}.[id], c) \in (map drop_last_of4 BP.bb1{2}) =>
        Some (oget (onth (odflt [] BP.vmap{1}.[id]) 
             (find (fun (x : cipher*(cipher*signature)*vote*vote) 
              => x.`1 = (nth witness BP.bb0{2} (index (id, oget BP.pubmap{2}.[id], c) 
                        (map drop_last_of4 BP.bb1{2}))).`3) 
                  (odflt [] BP.vmap{1}.[id])))).`4
          = Some (oget (onth (odflt [] BP.vmap{2}.[id]) 
                 (find (fun (x : cipher*(cipher*signature)*vote*vote) => x.`1 = c)
  ...
  /\ (forall i, 
         0 <= i < size BP.bb'{1} 
         => (nth witness (map drop_last_of3 BP.bb{2}) i) \in (map drop_last_of3 (rem_ids BP.bb1{1})) 
            => (nth witness (map drop_last_of3 BP.bb'{1}) i) \in (map drop_last_of3 (rem_ids BP.bb0{1})))

  /\ (forall i, 
         0 <= i < size BP.bb'{1} 
         => (nth witness (map drop_last_of3 BP.bb{2}) i) \in (map drop_last_of3 (rem_ids BP.bb1{1})) 
             => (nth witness (map drop_last_of3 BP.bb'{1}) i) 
                 = nth witness (map drop_last_of3 (rem_ids BP.bb0{1})) 
                               (index (nth witness (map drop_last_of3 BP.bb{2}) i) 
                                      (map drop_last_of3 (rem_ids BP.bb1{2}))))
...
while{1} ( ={BP.bb, BP.bb0, BP.bb1} 
           ...
           /\ (forall i, 
                 i < size bb'{1} => 
                 !((nth witness (map drop_last_of3 BP.bb{2}) i) \in (map drop_last_of3
                                                                    (rem_ids BP.bb1{1})))
           ...
           /\ (forall i, i < size bb'{1} 
                     => (nth witness bb'{1} i).`1 = (nth witness BP.bb{2} i).`1)

           /\ (forall i, 
                0 <= i < size bb'{1} =>
                (nth witness (map drop_last_of3 BP.bb{2}) i) \in (map drop_last_of3 (rem_ids BP.bb1{1})) 
             => (nth witness(map drop_last_of3 bb'{1}) i \in  (map drop_last_of3 (rem_ids BP.bb0{1}))))

           /\ (forall i, 
                0 <= i < size bb'{1} =>
                (nth witness (map drop_last_of3 BP.bb{2}) i) \in (map drop_last_of3 (rem_ids BP.bb1{1}))
                     => (nth witness (map drop_last_of3 bb'{1}) i) 
                         = nth witness (map drop_last_of3 (rem_ids BP.bb0{1})) 
                                       (index (nth witness (map drop_last_of3 BP.bb{2}) i) 
                                              (map drop_last_of3 (rem_ids BP.bb1{2}))))
  ...
  + smt(size_cat).
  + rewrite nth_cat.   
    move: H10; rewrite size_cat //=.
    case (i1 < size bb'{hr}). smt().
    move => Hn Hi.
    have Hx: i1 = size bb'{hr} by smt().
    rewrite Hx //=. 
    move: H11; rewrite -Hx. 
    rewrite (nth_map witness); first by smt().
    rewrite /drop_last_of3 //= Hx //=.
    smt(). (* contradiction *)
  + move: H10; rewrite size_cat //=; move => H10. 
    rewrite nth_cat.   
    case (i1 < size bb'{hr}). smt().
    move => Hn.
    have Hx: i1 = size bb'{hr} by smt().
    rewrite Hx //=. 
    move: H9.
    pose x:= (nth witness BP.bb{m0} (size bb'{hr})).
    move=> H9x.
    have ->: (forall i, (nth witness (rem_ids BP.bb0{m0}) i).`1 = 
                        (nth witness (rem_ids BP.bb1{m0}) i).`1).
    + move => ix.
      rewrite /rem_ids.
      case (0 <= ix && ix < size BP.bb0{m0}) => Hib.
      + rewrite (nth_map witness _ _ _ _ Hib).
        rewrite (nth_map witness); first by rewrite -H1 Hib.
        by rewrite /rem_id //= (H4 ix).    
      smt(@List).
    move: H9x; pose L:= (rem_ids BP.bb1{m0}); move => H9x.
    have Ho:= nth_map witness witness VSsmv.drop_last_of3 (index (x.`1, x.`2) (map VSsmv.drop_last_of3 L))
            L _.
    + rewrite index_ge0 //=. 
      by rewrite -(size_map VSsmv.drop_last_of3) index_mem H9x. 
    have Hod: (VSsmv.drop_last_of3 (nth witness L (index (x.`1, x.`2) (map VSsmv.drop_last_of3 L)))).`1
             = (nth witness L (index (x.`1, x.`2) (map VSsmv.drop_last_of3 L))).`1
      by rewrite /drop_last_of3. 
    rewrite -Hod -Ho.
    by rewrite nth_index 1: H9x.
  ...
  + move: H11; rewrite size_cat //=; move => H11.
    ...
    move => Hn.
    have Hx: i1 = size bb'{hr} by smt().
    move: H12; rewrite size_map -Hx //=. 
    move => H12. 
    have ->: (nth witness (map Top.drop_last_of3 BP.bb{m0}) i1)
  ...
  + move: H11; rewrite size_cat //=; move => H11.
    ...
    have Hx: i1 = size bb'{hr} by smt().
    move: H12 H9; rewrite size_map -Hx //=.
    rewrite (nth_map witness witness drop_last_of3). smt().
    smt(). 
  + smt().
...
+ smt(). 
+ smt(). 
+ case (i1 < size bb'_L) => Hi. 
  + by rewrite (H20 i1 Hi H24). 
  smt(nth_out). 
+ smt(@List). 
...
seq 3 3 : (
  ={glob A, glob S, glob C, glob CP, glob Ev, glob HRO.ERO, glob Ve, 
    BP.sk, BP.setHcheck,  BP.pk, BP.setidents, rL, BP.r, valid, BP.bb0, BP.bb1, 
    BP.bb, BP.sethappy, BP.setchecked, BP.pubmap, BP.secmap}
...
seq 13 13 : (((={glob A, glob S, glob C, glob CP, glob Ev, glob HRO.ERO, 
                ...
                glob Ve, BP.sk, BP.setHcheck, BP.pi', rL, BP.r, valid, BP.bb0, BP.pubmap, BP.secmap,
...
wp;call(_: ={glob HRO.ERO, glob S, BP.bb0, BP.bb1});[proc;inline*;auto | sim | sim | auto;progress]. 
call(_: ={glob HRO.ERO, glob S}); [1..2: by sim].
call(_:true).  
auto;progress. 
...
seq 1 1 : (
  (((={glob A, glob S, glob C, glob CP, glob Ev, glob HRO.ERO, BP.d, BP.pubmap, BP.secmap, BP.setidents, 
         BP.bbstar, valid', BP.pk, glob Ve, BP.sk, BP.setHcheck, BP.pi', rL,
         BP.r, valid, BP.bb0, BP.bb1, BP.bb, BP.sethappy, BP.setchecked, da} /\
     fdom BP.vmap{1} = fdom BP.vmap{2} /\
     forall (id0 : ident),
       (oget (ohead (odflt [] BP.vmap{1}.[id0]))).`3 =
       (oget (ohead (odflt [] BP.vmap{2}.[id0]))).`3) /\
...
seq 15 15 : (
  ={glob HRO.ERO, glob S, glob CP, BP.setchecked, BP.sethappy, BP.setHcheck,
    BP.bbstar, BP.bb, BP.pubmap, BP.secmap, ok, BP.setidents,  
...
wp;rnd. 
while(
  ={j, glob HRO.ERO, glob Ev, rL, BP.sk, BP.pk, BP.bb0, BP.bb1, BP.pubmap} /\ 0 <= j{1}
  /\ (size BP.bb'{1} = size BP.bb{2}) /\ size BP.bb0{2} = size BP.bb1{2}
  
  /\ (forall id pc c s, (id, pc, c, s) \in BP.bb0{1} => pc = oget BP.pubmap{1}.[id])
  /\ (forall id pc c s, (id, pc, c, s) \in BP.bb1{1} => pc = oget BP.pubmap{1}.[id])
  
  /\ (forall (id:ident) (c:cipher), (id, oget BP.pubmap{1}.[id], c) \in (map drop_last_of4 BP.bb0{1}) =>
        Some (oget (onth (odflt [] BP.vmap{1}.[id]) 
             (find (fun (x : cipher*(cipher*signature)*vote*vote) => x.`1 = c)
             (odflt [] BP.vmap{1}.[id])))).`4
         = dec_cipher_vo BP.sk{1}.`2 id c HRO.ERO.m{1}) 
  ...
  /\ (forall i, ! ((nth witness BP.bb{2} i).`1.`1, 
                   ...
                   (nth witness BP.bb{2} i).`2) \in (map drop_last_of4 BP.bb1{1}) 
         => nth witness BP.bb'{1} i = nth witness BP.bb{2} i) 

  /\ (forall i, (nth witness BP.bb'{1} i).`1 = (nth witness BP.bb{2} i).`1)

  /\ (forall (id0 : ident) (c : cipher), 
         (id0, oget BP.pubmap{2}.[id0], c) \in (map drop_last_of4 BP.bb1{2})
         =>  Some (oget (onth (odflt [] BP.vmap{1}.[id0]) 
                              (find (fun (x : cipher * (cipher*signature) * vote * vote) =>
  ...
  /\ (forall i, 
        0 <= i < size BP.bb'{1} 
        => nth witness (map drop_last_of3 BP.bb{2}) i \in (map drop_last_of3 (rem_ids BP.bb1{2}))
        => (nth witness (map drop_last_of3 BP.bb'{1}) i) 
            = nth witness (map drop_last_of3 (rem_ids BP.bb0{1})) 
                          (index (nth witness (map drop_last_of3 BP.bb{2}) i) 
                                 (map drop_last_of3 (rem_ids BP.bb1{2}))))
  ...
  /\ (forall pc c s, (pc, c, s) \in BP.bb{2} => (oget BP.pubmap{2}.[pc.`1]) = pc)
  /\ valid{2}
).
...
+ if{2}; last first. 
  + call(_: ={glob HRO.ERO}); first by sim. 
    auto =>/>;progress. 
    + smt(). 
    + have Hx: oget BP.pubmap{2}.[(nth witness BP.bb{2} j{2}).`1.`1] 
               = (nth witness BP.bb{2} j{2}).`1. 
      + by smt(@List). 
      move: H21; rewrite Hx; move => H21.
      by rewrite (H6 j{2} H21). 
    + smt().
    + smt().
    + smt().
    + smt().
  ...
  call{1}(Edec_Odec_vo gev sk.`2 id ev). 
  auto=>/>;progress. 
  + have Hx: oget BP.pubmap{2}.[(nth witness BP.bb'{1} j{2}).`1.`1] 
               = (nth witness BP.bb'{1} j{2}).`1. 
    + by smt(@List). 
    move: H20; rewrite Hx; move => H20.
    have Hy: oget BP.pubmap{2}.[(nth witness BP.bb{2} j{2}).`1.`1] 
               = (nth witness BP.bb{2} j{2}).`1. 
    + by smt(@List). 
    move: H21; rewrite Hy; move => H21.
    have := H9 j{2} _ _.
    + by rewrite H H18.
    + rewrite (nth_map witness). smt().
      move: H21; pose x:= nth witness BP.bb{2} j{2}.
      rewrite /rem_ids -map_comp.      
      rewrite /drop_last_of4 /drop_last_of3 /VSsmv.rem_id /(\o) //=.
      rewrite mapP; elim => y [#] Hymem Hy1 Hy2 Hy3.      
      rewrite mapP.
      exists y. smt().
    rewrite (nth_map witness). smt().
    rewrite mapP /drop_last_of3. elim => x [#] Hxmem Hxv1 Hxv2.
    move: Hxmem; rewrite (H11 x); move => Hxmem. 
    move: H20. rewrite mapP negb_exists //=; move=> H20.
    smt().
  + smt().
  + smt().    
  + smt(). 
...
+ exists* (glob Ev){2}, BP.sk{2}, id{2}, ev{2}; elim* => gev sk id ev. 
  call{2}(Edec_Odec_vo gev sk.`2 id ev). 
  auto =>/>;progress. 
  + have := H4 _ _ H20. 
    + have:= H5 j{2} _.
      rewrite (nth_map witness). smt().
      move: H21; pose y:= nth witness BP.bb{2} j{2}.
      rewrite /rem_ids -map_comp.      
      rewrite /drop_last_of4 /drop_last_of3 /VSsmv.rem_id /(\o) //=.
      rewrite mapP negb_exists //=. move=> H21.       
...
+ rewrite -(H8 _ _ H21).  
  rewrite -H7.  
  have ->: (nth witness BP.bb'{1} j{2}).`2 
           = (nth witness BP.bb0{2}
                  (index ((nth witness BP.bb'{1} j{2}).`1.`1,
                           oget BP.pubmap{2}.[(nth witness BP.bb'{1} j{2}).`1.`1],
  ...
  + have := H10 j{2} _ _.
    ...
    + rewrite (nth_map witness). smt().
      ...
      rewrite /rem_ids -map_comp.      
      rewrite /drop_last_of4 /drop_last_of3 /VSsmv.rem_id /(\o) //=.
      rewrite mapP //=. elim => x [#] Hxmem Hxv1 Hxv2 Hxv3. 
      rewrite mapP //=.
      exists x. rewrite Hxmem Hxv3 //=. 
      have := H16 y.`1 y.`2 y.`3 _.
      + rewrite /y; smt (@List).
      rewrite Hxv2. 
      smt().
    rewrite (nth_map witness). smt().
    ...
    + rewrite size_map H1.
      have ->: size BP.bb1{2} =
               size (map drop_last_of3 (rem_ids BP.bb1{2})).
      + by rewrite ?size_map.  
      rewrite index_mem.
      move: H21; pose y:= nth witness BP.bb{2} j{2}.
      rewrite /rem_ids -map_comp.      
   ...
   + rewrite H1.
     ...
     rewrite Hxv2. 
     smt().
   rewrite /drop_last_of3 /rem_id //=. move=> [Hxx1 Hxx2].
   rewrite Hxx2.

   have->: (index
              ( (nth witness BP.bb'{1} j{2}).`1.`1,
                oget BP.pubmap{2}.[(nth witness BP.bb'{1} j{2}).`1.`1],
                (nth witness BP.bb{2} j{2}).`2) 
              (map drop_last_of4 BP.bb1{2}))
         = (index 
              (nth witness (map Top.drop_last_of3 BP.bb{2}) j{2})
              (map Top.drop_last_of3 (rem_ids BP.bb1{2}))).
   + rewrite (H7 j{2}).   
     rewrite - map_comp.
     rewrite (nth_map witness). smt().
     pose x:= (nth witness BP.bb{2} j{2}).
     have ->: oget BP.pubmap{2}.[x.`1.`1] = x.`1.
     + rewrite (H16 x.`1 x.`2 x.`3 _). smt(@List).
       by done.
     have Hmem: forall (x: (ident * upkey * supkey * commitment) * cipher * signature), 
                   (x.`1.`1, x.`1, x.`2) \in (map drop_last_of4 BP.bb1{2})
                     <=> (drop_last_of3 x) \in (map Top.drop_last_of3 (rem_ids BP.bb1{2})).
     + rewrite -map_comp.
       rewrite /drop_last_of4 /drop_last_of3 /VSsmv.rem_id /(\o) //=.
       move => b. progress. 
       + (* => *) 
         rewrite mapP. 
         move: H22; rewrite mapP. elim => xb [#] Hxm Hx1 Hx2 Hx3. 
         exists xb. smt(). 
         (* <= *)
         rewrite mapP. 
         move: H22; rewrite mapP. elim => xb [#] Hxm Hx1 Hx2. 
         exists xb. smt().
     case ((x.`1.`1,x.`1,x.`2) \in map drop_last_of4 BP.bb1{2}).
     + smt(@List @SmtMap).   
     move=> Hxmm. smt(@List). 
   smt().
...
+ have Hmem: forall (x: (ident * upkey * supkey * commitment) * cipher * signature), 
  ...
  + rewrite -map_comp.
    ...
    + (* => *) 
      rewrite mapP. 
      move: H21; rewrite mapP. elim => xb [#] Hxm Hx1 Hx2 Hx3. 
      exists xb. smt(). 
    (* <= *)
    rewrite mapP. 
    move: H21; rewrite mapP. elim => xb [#] Hxm Hx1 Hx2. 
    exists xb. smt().
  case (! (nth witness (map Top.drop_last_of3 BP.bb{2}) i0 \in
           map Top.drop_last_of3 (rem_ids BP.bb1{2}))).
  + move => Hxmem.
    by rewrite (H6 i0 Hxmem).
  rewrite //=.
  search nth mem.
  move: H20; rewrite Hmem. 
  smt (@List).
+ smt().
+ smt().
+ smt().
...
while{1} ( ={BP.bb, BP.bb0, BP.bb1} 
           ...
           /\ (forall i, 
                 i < size bb'{1} => 
                 !((nth witness (map drop_last_of3 BP.bb{2}) i) \in 
                                (map drop_last_of3 (rem_ids BP.bb1{1})))
                     => nth witness bb'{1} i = nth witness BP.bb{2} i) 
           ...
           /\ (forall i, 
                 0 <= i < size bb'{1} =>
                     (nth witness (map drop_last_of3 BP.bb{2}) i) \in 
                     (map drop_last_of3 (rem_ids BP.bb1{1})) 
                     => (nth witness(map drop_last_of3 bb'{1}) i \in  
                     (map drop_last_of3 (rem_ids BP.bb0{1}))))
  ...
  + move: H10; rewrite size_cat //=; move => H10. 
    ...
    + move => ix.
      ...
      smt(@List).
    move: H9x; pose L:= (rem_ids BP.bb1{m0}); move => H9x.
    have Ho:= nth_map witness witness VSsmv.drop_last_of3 (index (x.`1, x.`2) 
                      (map VSsmv.drop_last_of3 L))
            L _.
    + rewrite index_ge0 //=. 
      by rewrite -(size_map VSsmv.drop_last_of3) index_mem H9x. 
    have Hod: (VSsmv.drop_last_of3 (nth witness L (index (x.`1, x.`2) (map VSsmv.drop_last_of3 L)))).`1
...
local module G3R(Ev:PKEvo.Scheme, Ve:Verifier, C:ValidIndS, CP:CommitmentProtocol, 
    ...
    module O = {
      ...
      if (id \in BP.setH) {
        ...
        BP.vmap.[id] <- (b1, s1pr, v0, v0) :: (odflt [] BP.vmap.[id]);

        bb <- (id, oget BP.pubmap.[id], b1, a1) :: bb;
        }
      }
    ...
    proc main() : bool = {
      var i;
      var id, upk, ct, d, da, vt, supk, susk;
      var valid, valid';
      var rL, j, b, ev, v, tr', sv, k;
...
seq 14 13 : ( ={glob CP, glob A, glob C, glob Ve, glob S, glob SS, glob Ev, glob HRO.ERO, 
                BP.pk, BP.sk, BP.bb, BP.setidents, BP.setHcheck, BP.secmapD, BP.setD, 
                BP.setidents, BP.setH, BP.vmap, BP.pubmap, BP.secmap, BP.setchecked, BP.sethappy}
  /\ (BP.pk{1}.`1 = get_epk BP.sk{1}.`2)
  /\ (G3R.bb{2} = BP.bb1{1})
  /\ (forall id pc c s, 
          (id, pc, c, s) \in G3R.bb{2} 
           => pc = oget BP.pubmap{2}.[id])
  /\ (forall id pc pc' c s s', 
          (id, pc, c, s) \in G3R.bb{2} 
          /\ (id, pc', c, s') \in G3R.bb{2} 
          => pc = pc')
  /\ (forall id c s, 
          (id, oget BP.pubmap{2}.[id], c, s) \in G3R.bb{2} 
          => !(exists pc, pc <> oget BP.pubmap{2}.[id] 
               /\ (id, pc, c, s) \in G3R.bb{2}))).

call(_: ={glob Ev, glob HRO.ERO, glob S, glob SS, BP.setidents, BP.setH, BP.pk, BP.sk, 
          BP.secmap, BP.vmap, BP.pubmap}
       /\ BP.bb1{1} = G3R.bb{2}
       /\ BP.pk{1}.`1 = get_epk BP.sk{1}.`2
       /\ (forall id pc c s, 
              (id, pc, c, s) \in G3R.bb{2} 
       ...
       /\ (forall id c s, 
               (id, oget BP.pubmap{2}.[id], c, s) \in G3R.bb{2} 
              => !(exists pc, pc <> oget BP.pubmap{2}.[id] /\ (id, pc, c, s) \in G3R.bb{2}))).
  
  (* ovote *)
  + proc. 
    if => //;inline*;wp.
    seq 10 0 : ( ={id, v0, v1, glob Ev, glob HRO.ERO, BP.pk, BP.sk, glob S, glob SS, 
                  BP.setidents, BP.secmap,
...
while (={i, BP.setidents, glob CP, glob SS, BP.secmap, BP.pubmap, BP.setD, BP.secmapD, BP.pk, BP.sk});
first by sim. 
wp;inline*.
wp;while(={i0, BP.setidents, tpk, tsk, pPk, pTr, pCo, pOp, trL, pi2}); first sim.  
wp;while(={i0, BP.setidents, trL, pTr, glob CP, pPk, tpk}); first sim. 
...
seq 6 6 : (
  ={glob A, glob S, glob C, glob CP, glob Ev, glob SS, glob HRO.ERO, glob Ve, 
    BP.sk, BP.setHcheck,  BP.pk, BP.setidents, rL, BP.r, valid, BP.bb, BP.sethappy, 
    BP.setchecked, BP.pubmap, BP.secmap, BP.vmap}
 /\ BP.bb1{1} = G3R.bb{2}
);last by sim. 
...
while(={j, BP.bb, glob HRO.ERO, BP.pk, BP.sk, glob Ev, rL, BP.vmap, BP.pubmap}
      /\ BP.bb1{1} = G3R.bb{2}
      /\ (forall id pc c s, 
            (id, pc, c, s) \in G3R.bb{2} 
             => pc = oget BP.pubmap{2}.[id])
...
seq 41 44 : (
  ={glob CP, glob A, glob C, glob Ve, glob S, glob SS, glob Ev, 
    BP.setHcheck, BP.secmapD, BP.setD, BP.setidents, BP.pk, 
    BP.bb, valid, trL, BP.setidents, BP.setHcheck, BP.setH, BP.secmapD, 
    BP.setD, BP.sethappy, BP.setchecked,
    ...
    /\ (forall id pc c s, (id, pc, c, s) \in BCCA.bb{2} => pc = oget BP.pubmap{2}.[id]) 

    /\ (forall (id:ident) (c:cipher) pc s, (id, pc, c, s) \in BCCA.bb{2}
           => is_some BP.vmap{2}.[id])

    /\ (forall id pc c s, (id, pc, c, s) \in BCCA.bb{2} => (c, id) \in BS.encL{2})
  ...
  /\ BP.setidents{1} = BP.setH{1} `|` BP.setD{1}
   ...
   /\ (forall id c, (c, id) \in BS.encL{2} => exists pc s, (id, pc, c, s) \in BCCA.bb{2})

  /\ (forall id c, (id, oget BP.pubmap{2}.[id], c) \in (map drop_last_of4 BCCA.bb{2}) <=> 
           (c, id) \in BS.encL{2})

  /\ (forall id c, !(exists pc s, (id, pc, c, s) \in BCCA.bb{2}) => !( (c,id) \in BS.encL{2}))
...
(*** A.a1 call ***)
call(_: 
  ={glob C, glob CP, glob Ev, glob HRO.ERO, glob S, glob SS, 
    BP.pk, BP.setH, BP.vmap, BP.pubmap, BP.secmap, BP.setidents}
  /\ (!BS.decQueried{2})
  ...
  /\ (forall id pc c s, (id, pc, c, s) \in BCCA.bb{2} => is_some BP.vmap{2}.[id])
...
+ proc. 
  ...
  sp; wp. 
  exists* (glob SS){1}, BP.sk{1}, BP.pk{1}, id1{1}, v2{1}. elim* => gss sk pk id v. 
  pose kp := (pk.`1 = get_epk sk.`2). 
  have em_kp : kp \/ !kp by exact/orbN.   
  elim em_kp.
  + move => h. 
    call(: true).
    wp; call (Eenc_dec_vo sk.`2 pk.`1 id v h). 
    auto=>/>; progress. 
    + smt (size_cat). 
    + smt(). 
    + case (id2 = id) => ideq.  
      + elim: H16 => Hx.
        + rewrite get_setE. smt(@List). 
        smt(get_setE).
      smt(get_setE).
    + elim: H16; first 2 by smt(@List).    
    + case (id2 = id) => ideq.  
      + elim: H16 => Hx.
        + rewrite get_setE. smt(@List). 
        smt(get_setE).
      smt(get_setE).
    + move: H16; rewrite mem_cat //=. 
      rewrite get_setE. smt (@List).
    + move: H16. rewrite mem_cat //=; move => H16.
      elim: H16. move => Hcmem.
      have:= (H9 _ _ Hcmem).
      smt().
      move => Hxm.
      exists (oget BP.pubmap{2}.[id]) result_R0.
      smt().
    + elim: H16; first 2 by smt(mem_cat).
    + move: H16; rewrite mem_cat //=. 
      elim => hx. smt().
      smt(). 
    + move: H16. rewrite ?negb_exists //=; move=> H16.
      have := H16  (oget BP.pubmap{2}.[id]).  
      rewrite negb_exists //=.
      move=> Hx.
      have := Hx result_R0. 
      rewrite mem_cat //=. smt(@List).
    + elim: H16. smt(get_setE).
      case (id2 <> id) => ideq.  
      + rewrite get_setE ideq //=. 
        exact H12.
      move: ideq; rewrite //=; move=> Hideq.
      rewrite -Hideq get_set_sameE.
      case (result_R = c1) => Hxc.
      + rewrite Hxc //=.
        by rewrite Hideq -Hxc H15.
      rewrite Hxc //=.
      have ->: !( find (fun (x : cipher * (cipher * signature) * vote * vote)
                        => x.`1 = c1) (odflt [] BP.vmap{2}.[id2]) 
                 + 1 = 0) by smt(find_ge0).
      rewrite //=.
      by exact H12.
    + move: H16; rewrite mem_cat //=; move => H16.  
  ...
  + move => h. 
    ...
    call(: true).
    wp; call(_: ={glob HRO.ERO}); first by sim. 
    by skip;progress. 
...
while(={i, BP.setidents, BP.secmap, BP.secmapD, BP.setD, BP.pubmap, glob CP, glob SS, trL}
  /\ (BP.pk{1}.`3.`4 = pPk{2})
  /\ (BP.pk{1}.`3.`5 = pCo{2})
  /\ (BP.sk{1}.`1 = pOp{2})
  /\ (forall j id, 
         j <= i{1} => id \in (take j (elems BP.setidents{1})) => is_some BP.pubmap{1}.[id])
  /\ (forall id, is_some BP.pubmap{1}.[id] => (oget BP.pubmap{1}.[id]).`1 = id)
).
...
seq 5 11 : (
  ={glob HRO.ERO, glob A, glob S, glob Ve, glob SS, valid, BP.setHcheck, 
    BP.vmap, BP.r, BP.bb, BP.setchecked, BP.sethappy, BP.pk, BP.secmap, BP.setidents, 
    BP.pubmap, glob CP}
  /\ (G2L.bb{1} = BCCA.bb{2})
  /\ (BP.sk{1}.`2 = BS.sk{2})
); last first. 
...
seq 13 13 : (
  ={glob HRO.ERO, glob A, glob S, glob SS, glob Ve, valid, valid', 
    BP.setHcheck, BP.vmap, BP.bbstar, da, BP.pi, BP.setidents, 
    BP.r, BP.bb, BP.setchecked, BP.sethappy, BP.pk, BP.secmap, BP.pubmap, glob CP}
...
while(={j, BP.bb, rL, BP.vmap, glob HRO.ERO, BP.pk, BP.pubmap} /\ 0 <= j{2}
  /\ (BP.sk{1}.`2 = BS.sk{2})
  /\ (BP.pk{1}.`1 = BS.pk{2}) 
  /\ (G2L.bb{1} = BCCA.bb{2})
  /\ (BP.sk{1}.`3 = tsk{2})
  ...
  /\ (mL{2} = map (fun (b:cipher*ident) =>  
              ...
              (map (fun (x:(ident*upkey*supkey*commitment)*cipher*signature) => 
                        (x.`2, x.`1.`1)) BP.bb{2} ))
).
sp 3 3. 
wp 1 1. 
...
seq 41 44 : (
  ={glob CP, glob A, glob C, glob Ve, glob S, glob SS, glob Ev, 
    ...
    BP.setidents, BP.setHcheck, BP.setH, BP.secmapD, BP.setD, BP.vmap, BP.sethappy, 
    BP.setchecked, BP.pubmap, BP.secmap, glob HRO.ERO}
  ...
  /\ (forall id c, 
        (id, oget BP.pubmap{2}.[id], c) \in (map drop_last_of4 BCCA.bb{2}) 
        <=> (c, id) \in BS.encL{2})
...
call(_: 
  ={glob C, glob CP, glob Ev, glob HRO.ERO, glob S, glob SS, 
    BP.vmap, BP.pk, BP.setH, BP.pubmap, BP.setidents, BP.secmap}
  /\ (!BS.decQueried{2})
  ...
  + move => h. 
    ...
    + case (id2 = id) => ideq.  
      + elim: H14 => Hx.
        ...
        smt(get_setE).
      smt(get_setE).
    + elim: H14; first 2 by smt(@List).    
    + case (id2 = id) => ideq.  
      + elim: H14 => Hx.
        + rewrite get_setE. smt(@List). 
        smt(get_setE).
      smt(get_setE).
    + move: H14; rewrite mem_cat //=. 
      rewrite get_setE. 
      smt (@List).
    + move: H14. rewrite mem_cat //=; move => H16.
      elim: H16. move => Hcmem.
      have:= (H9 _ _ Hcmem).
      smt().
      move => Hxm.
      exists (oget BP.pubmap{2}.[id]) result_R0.
      smt().
    + elim: H14; first 2 by smt(mem_cat).
    + move: H14; rewrite mem_cat //=. 
      elim => hx. smt().
      smt(). 
    + move: H14. rewrite ?negb_exists //=; move=> H16.
      have := H16  (oget BP.pubmap{2}.[id]).  
      rewrite negb_exists //=.
      move=> Hx.
      have := Hx result_R0. 
...
while(={i, BP.setidents, BP.secmap, BP.secmapD, BP.setD, BP.pubmap, glob CP, glob SS}
  /\ (BP.pk{1}.`3.`4 = pPk{2})
  /\ (BP.pk{1}.`3.`5 = pCo{2})
  /\ (BP.sk{1}.`1 = pOp{2})
  /\ (forall j id, j <= i{1} => id \in (take j (elems BP.setidents{1})) => is_some BP.pubmap{1}.[id])
...
(*** If sentences are equivalent ***)
seq 5 11 : (
  ={glob HRO.ERO, glob A, glob S, glob SS, glob Ve, valid, 
    BP.setHcheck, BP.r, BP.bb, BP.setchecked, BP.sethappy, BP.pk, 
    BP.secmap, BP.vmap, BP.setidents, 
    BP.pubmap, glob CP}
  /\ (G3R.bb{1} = BCCA.bb{2}) 
  /\ (BP.sk{1}.`2 = BS.sk{2})
);last  by sim. 
