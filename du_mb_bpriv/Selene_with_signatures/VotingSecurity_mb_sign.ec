...
require (*  *) VotingDefinition_mb_sign.

clone include VotingDefinition_mb_sign. 
...
module BP = {
  ...
  var bb         : (pubcred * ballot * saux) list (* first bb created by adversary *)
  var bbstar     : result * prf (* extra information produced by the adversary *)
  var bb'        : (pubcred * ballot * saux) list (* recovered board for tally when d = 1 *)
  var bb0        : (ident * pubcred * ballot * saux) list (* left world extended bulletin board *)
  var bb1        : (ident * pubcred * ballot * saux) list (* right world extended bulletin board *)
  ...
  var pc_to_id   : (pubcred, ident) fmap (* map from public credential to identity *)
...
op rem_id (x : ident * pubcred * ballot * saux) = (x.`2, x.`3, x.`4). 

(* Operator removing identity from all elements of an extended bulletin board *)
op rem_ids (bb:(ident * pubcred * ballot * saux) list) 
   = map rem_id bb. 

(* Drop 3rd element of a triple *)
op drop_last_of3['a 'b 'c] (x: 'a * 'b * 'c) = (x.`1, x.`2).


(**** The recovery algorithm ****)
module type Recover' = {
  proc rec(bb0 : (pubcred * ballot * saux) list, 
           bb1 : (pubcred * ballot * saux) list, 
           bb  : (pubcred * ballot * saux) list) 
           : (pubcred * ballot * saux) list
}.
...
module Recover' = {
  proc rec( bb0 :  (pubcred * ballot * saux) list, 
    ...
    while (i < size bb) {          (* for (p, b) ∈ BB: *)
      (p,b,a) <- nth witness bb i;   (* for (p, b) ∈ BB: *)
      if ( (p,b) \in map drop_last_of3 bb1 ) {       (* if ∃ j, id. BB1[j] = (id,(p, b)): *)
        j <- index (p,b) (map drop_last_of3 bb1);    (* if ∃ j, id. BB1[j] = (id,(p, b)): *)
...
module DU_MB_BPRIV_oracles(V:VotingSystem, H: HOracle.POracle, 
  ...
  (* Vote oracle constructing a ballot, a signature and a voter state used for verification *)
  proc vote_core(id, v0, v1, pb0, pb1, sl0, sl1) 
        : (state_pre * state_post * ballot * saux * pubcred * 
           state_pre * state_post * ballot * saux * pubcred) = {
    var p0, b0, a0, s0_pre, s0_post;
    var p1, b1, a1, s1_pre, s1_post;

    (p0, b0, a0, s0_pre, s0_post) <@ V(H, G).vote(BP.pk, id, pb0, sl0, v0);
    (p1, b1, a1, s1_pre, s1_post) <@ V(H, G).vote(BP.pk, id, pb1, sl1, v1);

    return (s0_pre, s0_post, b0, a0, p0, s1_pre, s1_post, b1, a1, p1);
  }
  
  (* Vote oracle calling vote_core and storing the ballots and signatures in the ballot box *)
  (* and the voter states in vmap                                                           *)
  proc vote(id:ident, v0 v1 : vote) : unit = {
    var p0, b0, a0, s0_pre, s0_post;
    var p1, b1, a1, s1_pre, s1_post;
    var l_or_r;
    ...
    if ((id \in BP.setH)) {
      (s0_pre, s0_post, b0, a0, p0, s1_pre, s1_post, b1, a1, p1) <@ 
      ...
      BP.bb0 <- (id, p0, b0, a0) :: BP.bb0;
      BP.bb1 <- (id, p1, b1, a1) :: BP.bb1;
    }
  }
  ...
  (* Returns the public part of bb0 or bb1 *)
  proc board() : pubBB = {
    var l_or_r, bb;
