...
module type Extractor(H:HOracle.POracle) = {
  proc extract(b: ((ident * upkey * commitment) * cipher), 
...
module SConsis1(V:VotingSystem, Ex:Extractor, H:HOracle.Oracle, 
                G:GOracle.Oracle) = {
  module V = V(H,G)
  module Ex = Ex(H)

  proc main(pc:(ident * upkey * commitment), v:vote) : bool = {
    ...
    (* create ballot for voter id and vote v *)
          b       <@ V.vote(BP.pk, id, pc, oget BP.sk.`1.[id], v);

    (* extract tracker and vote from ballot *)
       (id',v') <@ Ex.extract((b.`1,b.`2), BP.sk);
...
module type SConsis2_adv(O:SCons_Oracle) = {
  proc main(pd : (epkey * pkey * aux)) : 
                 ((ident * upkey * commitment) * cipher) list
}. 

module SConsis2_L(V:VotingSystem, C:ValidIndS, A:SConsis2_adv, 
...
module SConsis2_R(V:VotingSystem, C:ValidIndS, A:SConsis2_adv, 
                  H:HOracle.Oracle, G:GOracle.Oracle) = {
...
module SConsis3_L(V:VotingSystem, C:ValidIndS, A:SConsis3_adv, 
                  H:HOracle.Oracle, G:GOracle.Oracle) = {
...
module SConsis3_R(V:VotingSystem, Ex:Extractor, C:ValidIndS, 
                  A:SConsis3_adv, H:HOracle.Oracle, G:GOracle.Oracle) = {
...
declare axiom validInd_ax (gc:(glob C)) (pd1:epkey) (b:(ident*upkey*commitment)*cipher) : 
...
lemma sconsis1 (pc : ident * upkey * commitment) (v : vote) &m : 
    Pr[PKEvo.Correctness(Ev,H).main(v,pc.`1) @ &m : res] <=
    Pr[SConsis1(Selene(Ev,P,Ve,C,CP),Extract(Ev),H,G).main(pc,v) @ &m : res]. 
proof. 
byequiv(_: ={glob Ev, glob H} /\ arg{1}.`1 = arg{2}.`2 /\ arg{1}.`2 = arg{2}.`1.`1 ==> _) => //.
proc;inline*. wp. call(_: ={glob H}); first sim. 
...
equiv sconsis2 :
      SConsis2_L(Selene(Ev,P,Ve,C,CP),C,Asc2,HRO.ERO,G).main ~ 
      SConsis2_R(Selene(Ev,P,Ve,C,CP),C,Asc2,HRO.ERO,G).main : 
      ={glob H, glob G, glob Ev, glob C, glob CP, glob Asc2, BP.setidents} ==> res{1} => res{2}. 
proof. 
  proc.
  seq 5 5: ( ={ glob HRO.ERO, glob G, glob Ev, glob C, glob CP, glob Asc2,
...
lemma sconsis2_pr &m : 
    Pr[SConsis2_L(Selene(Ev,P,Ve,C,CP),C,Asc2,HRO.ERO,G).main() @ &m : res] <=
    Pr[SConsis2_R(Selene(Ev,P,Ve,C,CP),C,Asc2,HRO.ERO,G).main() @ &m : res]. 
proof. 
by byequiv sconsis2. 
qed. 
...
equiv consis3 &m :
  SConsis3_L(Selene(Ev,P,Ve,C,CP), C, Asc3, H, G).main ~ 
  SConsis3_R(Selene(Ev,P,Ve,C,CP), Extract(Ev), C, Asc3, H, G).main : 
...
seq 26 28 : (
  ={glob H, glob G, glob C, glob CP, glob Ev, glob Asc3,  
    BP.pk, BP.sk, bb, BP.setidents, bv, vtL} /\  (vtL't{2} = [])
); first wp;sim.   
...
seq 5 1 : (
  ={glob H, glob G, glob C, glob CP, glob Ev, glob Asc3, 
    BP.pk, BP.sk, bb, BP.setidents, bv, vtL}
...
seq 1 1 : (
  ={glob H, glob G, glob C, glob CP, glob Ev, glob Asc3, 
    BP.pk, BP.sk, BP.setidents, bv, vtL}
  /\ i1{1} = i{2} /\ vtL't{2} = rL{1} /\ pd{1} = BP.pk{1}
).

while (={BP.pk, BP.sk, glob Ev, glob H} 
    
       /\ bb0{1} = bb{2} /\ i1{1} = i{2} /\ vtL't{2} = rL{1}
