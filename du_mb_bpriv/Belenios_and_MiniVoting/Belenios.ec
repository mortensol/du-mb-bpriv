...
require (*  *) DiffieHellman MiniVotingSecurity_mb.
...
(*** Labelled Public-Key Encryption Scheme
  ...
  (* this is what we need type label = unit. *)
  type ident.
  type label = ident.
...
clone export MiniVotingSecurity_mb as MV2 with
  type ident         <- ident,  
  type vote          <- ptxt,  
  (* use LPKE types *)
  type cipher        <- cipher, 
  ...
  type skey          <- skey,   
  type VSmv.h_in     <- h_in,   
  type VSmv.h_out    <- h_out,
  type result        <- ptxt list,
  type VSmv.pubBB    <- (ident * cipher) list,
  op VSmv.dh_out     <- dh_out, 
  op n               <- n,          
  op dec_cipher      <- dec_cipher,        
  op getPK           <- getPK,
  op Count           <- Count,
  op VSmv.Publish    <- Publish
  proof is_finite_h_in, distr_h_out, n_pos.
    realize is_finite_h_in by rewrite h_in_finite. 
    realize distr_h_out by rewrite dh_out_ll. 
    realize n_pos by rewrite n_pos. 
...
module (IPKE(P: PSpke.Prover, Ve: PSpke.Verifier): Scheme) (O:HOracle.POracle) = {
 ...
 module (CV (Ve: PSpke.Verifier) :ValidInd) (O:HOracle.POracle) ={
   proc validInd (b: label * cipher, pk: pkey): bool ={
   var ev;
   
   ev <@ Ve(O).verify((to_stm pk b.`1 b.`2.`1), b.`2.`2);
   return ev;
   }
 }.
...
lemma du_mb_bpriv &m :
  ...
  => 
  `|Pr[DU_MB_BPRIV_L(Belenios (Pe,Ve,Vc,Pz,Vz),A,HRO.ERO,G).main() @ &m : res] 
    - Pr[DU_MB_BPRIV_R(Belenios (Pe,Ve,Vc,Pz,Vz),A,HRO.ERO,G,S,Recover').main() @ &m : res]|
   <=  
...
proof. 
  move => IHD.
  have ->: Pr[DU_MB_BPRIV_L(Belenios (Pe,Ve,Vc,Pz,Vz),A,HRO.ERO,G).main() @ &m : res]
  = Pr[DU_MB_BPRIV_L(MV(IPKE(Pe, Ve),Pz,Vz,CV(Vc)), A, HRO.ERO, G).main() @ &m : res]
    by byequiv =>/>; sim.

  have->: Pr[DU_MB_BPRIV_R(Belenios (Pe,Ve,Vc,Pz,Vz),A,HRO.ERO,G,S,Recover').main() @ &m : res]
  = Pr[DU_MB_BPRIV_R(MV(IPKE(Pe, Ve), Pz, Vz, CV(Vc)), A, HRO.ERO, G, S, Recover').main() @ &m : res]
    by byequiv =>/>; sim.

  by rewrite (du_mb_bpriv G (IPKE(Pe,Ve)) S Vz Pz R (CV(Vc)) A Gi_ll Go_ll A_a1_ll A_a2_ll 
               A_a3_ll A_a4_ll PS_p_ll PS_v_ll E_kg_ll E_enc_ll E_dec_ll Si_ll So_ll 
               Sp_ll R_m_ll C_vi_ll Rho_weight Ekgen_extractPk2 Edec_Odec2 Ee_eq2 Eenc_decL2 &m IHD).
qed.
